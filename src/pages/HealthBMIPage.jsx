...
function BMICalculator() {
  ...
  return (
    <div className="rounded-2xl border p-5 space-y-4" style={{ borderColor: `${COLOR}30`, background: `${COLOR}07` }}>
      <p className="text-sm font-bold uppercase tracking-widest" style={{ color: COLOR }}>Tính BMI & Đánh Giá Vòng Eo</p>
        ...
        {[{ l: 'Cân nặng (kg)', v: weight, s: setWeight, ph: 'VD: 70' }, { l: 'Chiều cao (cm)', v: height, s: setHeight, ph: 'VD: 170' }, { l: 'Vòng eo (cm)', v: waist, s: setWaist, ph: 'VD: 85' }].map(f => (
          <div key={f.l}>
            ...
            <input type="number" placeholder={f.ph} value={f.v} onChange={e => f.s(+e.target.value)}
              className="w-full rounded-xl border border-border bg-bg text-text text-base px-3 py-2 focus:outline-none" />
          </div>
        ))}
        <div>
          ...
          <div className="flex gap-4">
            ...
            {waist && (
              <div className="rounded-xl border p-3 flex-1 text-center" style={{ borderColor: `${waistRisk ? '#ef4444' : '#10b981'}30`, background: `${waistRisk ? '#ef4444' : '#10b981'}08` }}>
                ...
                <div className="text-sm font-bold" style={{ color: waistRisk ? '#ef4444' : '#10b981' }}>{waistRisk ? 'Nguy cơ cao' : 'Bình thường'}</div>
                <div className="text-sm text-muted">Vòng eo (cm)</div>
              </div>
            )}
            {whr && (
              <div className="rounded-xl border border-border p-3 flex-1 text-center">
                <div className="text-4xl font-black text-text">{whr}</div>
                <div className="text-sm text-muted">Tỷ lệ eo/chiều cao</div>
                <div className="text-sm" style={{ color: +whr > 0.5 ? '#ef4444' : '#10b981' }}>{+whr > 0.5 ? 'Trên ngưỡng' : 'OK'}</div>
              </div>
            )}
          </div>
          <div className="rounded-xl p-3" style={{ background: `${cat.color}10`, borderLeft: `3px solid ${cat.color}` }}>
            <p className="text-sm text-muted leading-relaxed"><strong style={{ color: cat.color }}>Gợi ý hành động:</strong> {cat.action}</p>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          ...
          <span className="inline-block text-sm font-bold uppercase tracking-widest mt-3 mb-4 px-3 py-1 rounded-full border" style={{ color: COLOR, background: `${COLOR}15`, borderColor: `${COLOR}30` }}>E1 · Chỉ Số Cơ Thể</span>
      ...
      <div className={`${ORBIT_CLASS} rounded-3xl p-[1.5px] mb-12`}>
        <div className="relative rounded-3xl overflow-hidden h-52 md:h-72">
          ...
          <div className="absolute bottom-4 left-6">
            <span className="text-sm font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: COLOR, background: 'rgba(10,10,10,0.6)', borderColor: `${COLOR}30` }}>Đo vòng eo 1 lần/tuần · Cân 1–3 lần/tuần</span>
          </div>
        </div>
      </div>
      ...
      <RevealBlock className="mb-10">
        ...
        <p className="text-muted text-base mb-6">Nhập số liệu để xem phân loại và gợi ý hành động</p>
        <BMICalculator />
      </RevealBlock>

      <RevealBlock className="mb-10">
        <h2 className="text-2xl md:text-3xl font-bold mb-1" style={{ color: COLOR }}>Phân Loại BMI & Hành Động</h2>
        <p className="text-muted text-base mb-6">BMI là chỉ số sàng lọc — cần kết hợp với vòng eo và các chỉ số khác để đánh giá toàn diện</p>
        <div className="space-y-3">
          {BMI_CATS.map(c => (
            <div key={c.label} className="rounded-2xl border border-border bg-surface p-4 flex gap-4">
              <div className="shrink-0 w-24">
                <div className="text-base font-black" style={{ color: c.color }}>{c.range}</div>
                <div className="text-sm font-bold" style={{ color: c.color }}>{c.label}</div>
              </div>
              <p className="text-base text-muted leading-relaxed">{c.action}</p>
            </div>
          ))}
        </div>
        <p className="text-sm text-muted mt-3">* BMI không phân biệt mỡ và cơ — người tập gym nhiều có thể BMI cao nhưng tỷ lệ mỡ thấp. Luôn kết hợp với vòng eo.</p>
      </RevealBlock>

      <RevealBlock className="mb-10">
        <h2 className="text-2xl md:text-3xl font-bold mb-1" style={{ color: COLOR }}>Vòng Eo — Chỉ Số Quan Trọng Hơn Cân Nặng</h2>
        <p className="text-muted text-base mb-6">Mỡ bụng nội tạng liên quan đến nguy cơ tim mạch, đái tháo đường type 2, gan nhiễm mỡ và hội chứng chuyển hóa</p>
            ...
            <div key={w.group} className="rounded-2xl border border-border bg-surface p-5">
              ...
              <div className="flex gap-3">
                <div className="flex-1 rounded-xl p-3 text-center" style={{ background: '#10b98115' }}>
                  ...
                  <div className="text-sm text-muted">{w.normal}</div>
                </div>
                <div className="flex-1 rounded-xl p-3 text-center" style={{ background: '#ef444415' }}>
                  ...
                  <div className="text-sm text-muted">{w.danger}</div>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="rounded-2xl border border-border bg-surface p-5">
          <p className="text-base font-bold text-text mb-3">Cách Đo Vòng Eo Đúng</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={c.t} className="rounded-xl border p-3" style={{ borderColor: `${c.c}25`, background: `${c.c}08` }}>
                <p className="text-sm text-muted mb-1">{c.t}</p>
                <p className="text-sm font-bold" style={{ color: c.c }}>{c.n}</p>
              </div>
            ))}
          </div>
        </div>
