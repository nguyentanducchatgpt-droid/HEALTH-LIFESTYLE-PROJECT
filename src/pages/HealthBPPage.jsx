  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <input value={sys} onChange={e => setSys(e.target.value)} placeholder="Tâm thu (mmHg)" type="number" className="flex-1 min-w-[120px] bg-bg border border-border rounded-xl px-3 py-2 text-sm text-text placeholder-muted" />
        <input value={dia} onChange={e => setDia(e.target.value)} placeholder="Tâm trương (mmHg)" type="number" className="flex-1 min-w-[120px] bg-bg border border-border rounded-xl px-3 py-2 text-sm text-text placeholder-muted" />
        <input value={pulse} onChange={e => setPulse(e.target.value)} placeholder="Nhịp tim (bpm)" type="number" className="flex-1 min-w-[100px] bg-bg border border-border rounded-xl px-3 py-2 text-sm text-text placeholder-muted" />
        <button onClick={addEntry} className="px-4 py-2 rounded-xl text-sm font-bold text-white" style={{ background: COLOR }}>+ Thêm</button>
      </div>
      {entries.length === 0 && <p className="text-muted text-sm text-center py-6">Chưa có dữ liệu. Thêm lần đo đầu tiên.</p>}
      <div className="space-y-2 max-h-64 overflow-y-auto">
        {entries.map((e, i) => (
          <div key={i} className="flex items-center gap-3 rounded-xl px-3 py-2 border border-border">
            ...
            <span className="font-bold text-sm" style={{ color: getColor(e.sys, e.dia) }}>{e.sys}/{e.dia}</span>
            {e.pulse && <span className="text-xs text-muted">♥ {e.pulse} bpm</span>}
            <span className="text-xs text-muted ml-auto">{e.date} {e.time}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div className="w-20 h-20 rounded-3xl text-5xl bg-surface border flex items-center justify-center shrink-0" style={{ borderColor: `rgba(${RGB},0.2)`, animation: 'float 3s ease-in-out infinite' }}>
          🫀
        </div>
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-text leading-tight">Huyết Áp</h1>
      ...
      {b0.age && (
        <RevealBlock delay={0} className="mb-10">
          <div className="rounded-2xl border p-4 md:p-5" style={{ borderColor: `rgba(${RGB},0.13)`, background: `rgba(${RGB},0.04)` }}>
            ...
            <p className="text-xs text-muted">{b0.age} tuổi · {b0.weight}kg — Nên đo huyết áp {b0.age >= 40 ? 'mỗi 3–6 tháng' : 'mỗi năm'} và theo dõi hàng ngày nếu có nguy cơ.</p>
          </div>
        </RevealBlock>
      )}
      ...
      <RevealBlock delay={1} className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Phân Loại Huyết Áp (AHA 2017)</h2>
        <p className="text-muted text-sm mb-6">Đơn vị mmHg. Áp dụng cho người lớn ≥ 18 tuổi, không dùng thuốc huyết áp.</p>
        <div className="space-y-3">
          {BP_CATS.map((c, i) => (
            <div key={i} className="rounded-2xl border border-border p-4 flex flex-col sm:flex-row sm:items-center gap-3" style={{ background: c.bg }}>
              ...
              <div className="flex gap-4 text-xs text-muted sm:w-40 shrink-0">
                <span>Tâm thu: <strong className="text-text">{c.sys}</strong></span>
                <span>Tâm trương: <strong className="text-text">{c.dia}</strong></span>
              </div>
              <p className="text-xs text-muted flex-1">{c.action}</p>
            </div>
          ))}
        </div>
        <div className="mt-4 rounded-2xl border p-4 text-sm" style={{ borderColor: `rgba(239,68,68,0.3)`, background: 'rgba(239,68,68,0.06)' }}>
      ...
      <RevealBlock delay={2} className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Kỹ Thuật Đo Đúng Chuẩn</h2>
        <p className="text-muted text-sm mb-6">Sai kỹ thuật có thể khiến kết quả lệch 10–20 mmHg.</p>
        <div className="grid sm:grid-cols-2 gap-4">
          {STEPS.map((s, i) => (
            <div key={i} className="rounded-2xl border border-border bg-surface p-4 flex gap-4">
              <div className="w-9 h-9 rounded-xl font-bold text-lg flex items-center justify-center shrink-0" style={{ background: `rgba(${RGB},0.12)`, color: COLOR }}>{s.num}</div>
              <div>
                <div className="font-bold text-sm text-text mb-1">{s.title}</div>
      ...
      <RevealBlock delay={3} className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Lối Sống Kiểm Soát Huyết Áp</h2>
        <p className="text-muted text-sm mb-6">Thay đổi lối sống có thể giảm HA 5–20 mmHg mà không cần thuốc.</p>
        <div className="grid sm:grid-cols-2 gap-4">
          {LIFESTYLE.map((l, i) => (
            <div key={i} className="rounded-2xl border border-border bg-surface p-4 flex gap-3 hover:border-red-500/30 transition-colors">
              ...
              <div>
                <div className="font-bold text-sm text-text mb-1">{l.title}</div>
      ...
      <RevealBlock delay={4} className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-4" style={{ color: COLOR }}>Ghi Nhật Ký Huyết Áp</h2>
        <BPJournal />
      </RevealBlock>
