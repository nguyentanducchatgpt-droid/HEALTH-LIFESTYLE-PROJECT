  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5">
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        <input value={sys} onChange={e => setSys(e.target.value)} placeholder="Tâm thu (mmHg)" type="number" className="flex-1 min-w-[120px] bg-bg border border-border rounded-xl px-3 py-2 text-base text-text placeholder-muted" />
        <input value={dia} onChange={e => setDia(e.target.value)} placeholder="Tâm trương (mmHg)" type="number" className="flex-1 min-w-[120px] bg-bg border border-border rounded-xl px-3 py-2 text-base text-text placeholder-muted" />
        <input value={pulse} onChange={e => setPulse(e.target.value)} placeholder="Nhịp tim (bpm)" type="number" className="flex-1 min-w-[100px] bg-bg border border-border rounded-xl px-3 py-2 text-base text-text placeholder-muted" />
        <button onClick={addEntry} className="px-4 py-2 rounded-xl text-base font-bold text-white" style={{ background: COLOR }}>+ Thêm</button>
      </div>
      {entries.length === 0 && <p className="text-muted text-base text-center py-6">Chưa có dữ liệu. Thêm lần đo đầu tiên.</p>}
      <div className="space-y-2 max-h-64 overflow-y-auto">
        {entries.map((e, i) => (
          <div key={i} className="flex items-center gap-3 rounded-xl px-3 py-2 border border-border">
            ...
            <span className="font-bold text-base" style={{ color: getColor(e.sys, e.dia) }}>{e.sys}/{e.dia}</span>
            {e.pulse && <span className="text-sm text-muted">♥ {e.pulse} bpm</span>}
            <span className="text-sm text-muted ml-auto">{e.date} {e.time}</span>
          </div>
        ))}
      </div>
    </div>
...
export default function HealthBPPage() {
  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      <Link to="/pillar/e" className="inline-flex items-center gap-2 text-base text-muted hover:text-text mb-8 transition-colors">
        ← Kiến Thức Sức Khỏe
      </Link>
      ...
      {b0.age && (
        <RevealBlock delay={0} className="mb-10">
          <div className="rounded-2xl border p-4 md:p-5" style={{ borderColor: `rgba(${RGB},0.13)`, background: `rgba(${RGB},0.04)` }}>
            ...
            <p className="text-sm text-muted">{b0.age} tuổi · {b0.weight}kg — Nên đo huyết áp {b0.age >= 40 ? 'mỗi 3–6 tháng' : 'mỗi năm'} và theo dõi hàng ngày nếu có nguy cơ.</p>
          </div>
        </RevealBlock>
      )}
      ...
      <RevealBlock delay={1} className="mb-12">
        ...
        <p className="text-muted text-base mb-6">Đơn vị mmHg. Áp dụng cho người lớn ≥ 18 tuổi, không dùng thuốc huyết áp.</p>
        <div className="space-y-3">
          {BP_CATS.map((c, i) => (
            <div key={i} className="rounded-2xl border border-border p-4 flex flex-col sm:flex-row sm:items-center gap-3" style={{ background: c.bg }}>
              ...
              <div className="flex gap-4 text-sm text-muted sm:w-40 shrink-0">
                <span>Tâm thu: <strong className="text-text">{c.sys}</strong></span>
                <span>Tâm trương: <strong className="text-text">{c.dia}</strong></span>
              </div>
              <p className="text-sm text-muted flex-1">{c.action}</p>
            </div>
          ))}
        </div>
        <div className="mt-4 rounded-2xl border p-4 text-base" style={{ borderColor: `rgba(239,68,68,0.3)`, background: 'rgba(239,68,68,0.06)' }}>
      ...
      <RevealBlock delay={2} className="mb-12">
        ...
        <p className="text-muted text-base mb-6">Sai kỹ thuật có thể khiến kết quả lệch 10–20 mmHg.</p>
        <div className="grid sm:grid-cols-2 gap-4">
          {STEPS.map((s, i) => (
            <div key={i} className="rounded-2xl border border-border bg-surface p-4 flex gap-4">
              ...
              <div>
                <div className="font-bold text-base text-text mb-1">{s.title}</div>
                <div className="text-sm text-muted">{s.desc}</div>
              </div>
            </div>
          ))}
        </div>
        <div className="mt-4 rounded-2xl border border-border bg-surface p-4 text-base text-muted">
      ...
      <RevealBlock delay={3} className="mb-12">
        ...
        <p className="text-muted text-base mb-6">Thay đổi lối sống có thể giảm HA 5–20 mmHg mà không cần thuốc.</p>
        <div className="grid sm:grid-cols-2 gap-4">
          {LIFESTYLE.map((l, i) => (
            <div key={i} className="rounded-2xl border border-border bg-surface p-4 flex gap-3 hover:border-red-500/30 transition-colors">
              ...
              <div>
                <div className="font-bold text-base text-text mb-1">{l.title}</div>
      ...
      <Link to="/pillar/e" className="inline-flex items-center gap-2 text-base text-muted hover:text-text transition-colors">
