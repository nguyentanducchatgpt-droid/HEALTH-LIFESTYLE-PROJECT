  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-text leading-tight">Khám Định Kỳ</h1>
      ...
      {b0.age && (
        <RevealBlock delay={0} className="mb-10">
          <div className="rounded-2xl border p-4" style={{ borderColor: `rgba(${RGB},0.13)`, background: `rgba(${RGB},0.04)` }}>
            ...
            <p className="text-xs text-muted">{b0.age} tuổi · Nên khám định kỳ {b0.age >= 40 ? 'mỗi 6–12 tháng với gói cơ bản và mở rộng phù hợp tuổi.' : 'hàng năm với gói cơ bản.'}</p>
          </div>
        </RevealBlock>
      )}
      ...
      <RevealBlock delay={1} className="mb-12">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-xl md:text-2xl font-bold" style={{ color: COLOR }}>Gói Khám Cơ Bản Hàng Năm</h2>
          <span className="text-sm font-bold" style={{ color: COLOR }}>{doneCount}/{BASIC_PACKAGE.length}</span>
        </div>
          ...
          {BASIC_PACKAGE.map((item, i) => (
            <button key={i} onClick={() => toggle(i)} className="w-full flex items-center gap-3 rounded-2xl border border-border bg-surface p-4 text-left hover:border-lime-500/30 transition-colors">
              ...
              <div className="flex-1">
                <span className={`text-sm ${checked.includes(i) ? 'line-through text-muted' : 'text-text'}`}>{item.item}</span>
              </div>
              <span className="text-xs text-muted shrink-0">{item.freq}</span>
            </button>
          ))}
        </div>
        <p className="text-xs text-muted mt-3">Nhấn để đánh dấu đã hoàn thành. Dữ liệu lưu trong thiết bị.</p>
      </RevealBlock>

      <RevealBlock delay={2} className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-2" style={{ color: COLOR }}>Gói Mở Rộng Theo Nguy Cơ</h2>
        <p className="text-muted text-sm mb-6">Trao đổi với bác sĩ về gói khám phù hợp với hồ sơ nguy cơ cá nhân của bạn.</p>
          ...
          {EXTENDED_PACKAGES.map((pkg, i) => (
            <div key={i} className="rounded-2xl border border-border bg-surface overflow-hidden">
              <button onClick={() => setOpenExtended(openExtended === i ? null : i)} className="w-full flex items-center gap-3 p-4 text-left hover:bg-white/5 transition-colors">
                ...
                <div className="flex-1">
                  <div className="font-bold text-sm text-text">{pkg.title}</div>
                </div>
                <span className="text-muted">{openExtended === i ? '▲' : '▼'}</span>
              </button>
              {openExtended === i && (
                <div className="px-4 pb-4 border-t border-border pt-3 space-y-3">
                  <p className="text-xs text-muted"><strong style={{ color: pkg.color }}>Áp dụng khi: </strong>{pkg.triggers}</p>
