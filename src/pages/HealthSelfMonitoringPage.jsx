...
function DailyCheckForm() {
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-5 space-y-4">
      <div>
        <label className="text-base text-text mb-1 block">Bạn ngủ bao nhiêu tiếng tối qua?</label>
        <div className="flex items-center gap-3">
          <input type="range" min={4} max={10} step={0.5} value={answers.sleep || 7} onChange={e => set('sleep', +e.target.value)} className="flex-1" />
          <span className="text-base font-bold w-12 text-right" style={{ color: COLOR }}>{answers.sleep || 7} giờ</span>
        </div>
      </div>
      {[
        ...
        <div key={i} className="flex items-center justify-between">
          <span className="text-base text-text">{item.q}</span>
      ...
      <div>
        <label className="text-base text-text mb-2 block">Tâm trạng hôm nay?</label>
  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      {b0.age && (
        <RevealBlock delay={0} className="mb-10">
          <div className="rounded-2xl border p-4" style={{ borderColor: `rgba(${RGB},0.13)`, background: `rgba(${RGB},0.04)` }}>
            ...
            <p className="text-sm text-muted">{b0.age} tuổi · {b0.weight}kg · {b0.height}cm — Bắt đầu theo dõi cân nặng, huyết áp và vòng eo hàng tuần để có baseline cá nhân.</p>
          </div>
        </RevealBlock>
      )}
      ...
      <RevealBlock delay={1} className="mb-12">
        ...
        <p className="text-muted text-base mb-6">Không nhất thiết theo dõi tất cả mỗi ngày — mỗi chỉ số có tần suất phù hợp riêng.</p>
        <div className="space-y-3">
          {METRICS.map((m, i) => (
            <div key={i} className="rounded-2xl border border-border bg-surface p-4">
              <div className="flex items-start gap-3 mb-2">
                <div className="flex-1">
                  <div className="font-bold text-base text-text">{m.label}</div>
                  <div className="text-sm text-muted mt-0.5">{m.when} · {m.tool}</div>
                </div>
                <span className="text-sm px-2 py-1 rounded-full shrink-0" style={{ background: `rgba(${RGB},0.12)`, color: COLOR }}>{m.freq}</span>
              </div>
      ...
      <RevealBlock delay={2} className="mb-12">
        ...
        <p className="text-muted text-base mb-4">5 câu hỏi đơn giản, mỗi ngày 30 giây. Dữ liệu lưu trong thiết bị của bạn.</p>
        <DailyCheckForm />
      </RevealBlock>
