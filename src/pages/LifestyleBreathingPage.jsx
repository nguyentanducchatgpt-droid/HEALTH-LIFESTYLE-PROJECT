...
function BoxBreathingTimer() {
  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-6 flex flex-col items-center gap-4">
      <div className="text-sm font-bold uppercase tracking-widest text-muted">Box Breathing Timer</div>
      <div className="relative w-40 h-40 flex items-center justify-center">
        ...
        <div className="text-center transition-transform duration-1000" style={{ transform: `scale(${running ? circleSize : 1})` }}>
          ...
          <div className="text-xs text-muted mt-1">{running ? currentPhase.label : 'Bắt đầu'}</div>
        </div>
      </div>
      {running && <div className="text-xs text-muted">Vòng {cycles + 1}</div>}
      <button onClick={startStop} className="px-6 py-2 rounded-full text-sm font-bold transition-all" style={{ background: running ? 'rgba(239,68,68,0.15)' : `rgba(${RGB},0.15)`, color: running ? '#ef4444' : COLOR, border: `1px solid ${running ? 'rgba(239,68,68,0.3)' : `rgba(${RGB},0.3)`}` }}>
    ...
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          ...
          <span className="inline-block text-xs font-bold uppercase tracking-widest mt-3 mb-4 px-3 py-1 rounded-full border" style={{ color: COLOR, background: `rgba(${RGB},0.1)`, borderColor: `rgba(${RGB},0.2)` }}>C6 · Breathing</span>
          <p className="text-muted text-base leading-relaxed max-w-2xl">Bạn thở 20.000 lần mỗi ngày. Đa số vô thức và không hiệu quả. Chỉ cần thay đổi cách thở 5 phút mỗi ngày, bạn có thể giảm cortisol, cải thiện giấc ngủ và tăng hiệu suất não bộ.</p>
        </div>
      </div>
      ...
      <div className={`${ORBIT_CLASS} rounded-3xl p-[1.5px] mb-12`}>
        <div className="relative rounded-3xl overflow-hidden h-52 md:h-72">
          ...
          <div className="absolute bottom-4 left-6">
            <span className="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: COLOR, background: 'rgba(0,0,0,0.6)', borderColor: `rgba(${RGB},0.2)` }}>Hít thở có chủ ý · 3 kỹ thuật khoa học</span>
          </div>
        </div>
      </div>
      ...
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Khoa Học Đằng Sau Hơi Thở</h2>
        <p className="text-muted text-sm mb-6">Thở là công cụ duy nhất bạn có thể điều khiển cả hệ tự chủ và ý thức.</p>
      ...
      {/* 3 techniques */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>3 Kỹ Thuật Thở Cốt Lõi</h2>
        <p className="text-muted text-sm mb-6">Mỗi kỹ thuật có mục đích khác nhau — chọn đúng thời điểm để hiệu quả tối đa.</p>
          ...
          <div className="rounded-2xl border p-5 md:p-6" style={{ borderColor: `${tech.color}30`, background: `${tech.color}08` }}>
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                ...
                <div className="text-xs font-bold uppercase tracking-widest mt-1" style={{ color: tech.color }}>{tech.subtitle}</div>
                <p className="text-sm text-muted mt-2 leading-relaxed">{tech.desc}</p>
              </div>
            </div>

            <div className="grid md:grid-cols-2 gap-5">
              <div>
                <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: tech.color }}>Cách Thực Hiện</div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: tech.color }}>Lợi Ích</div>
                  <ul className="space-y-1">
                    {tech.benefits.map((b, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-text"><span style={{ color: tech.color }}>✓</span>{b}</li>
                    ))}
                  </ul>
                </div>
                <div className="rounded-xl p-3" style={{ background: `${tech.color}10` }}>
                  <div className="text-xs text-muted mb-1">⏰ Khi nào dùng</div>
                  <div className="text-sm text-text font-medium">{tech.when}</div>
                  <div className="text-xs text-muted mt-1">🕐 {tech.duration}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Box Breathing Timer</h2>
        <p className="text-muted text-sm mb-6">Thực hành ngay — timer sẽ hướng dẫn từng bước theo nhịp 4-4-4-4.</p>
      ...
      {/* Daily plan */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Kế Hoạch Thở Hằng Ngày</h2>
        <p className="text-muted text-sm mb-6">Tích hợp 3 kỹ thuật vào thói quen hiện tại — không cần thêm thời gian riêng.</p>
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-4 rounded-xl p-4 border border-border bg-surface hover:border-sky-500/20 transition-colors">
              ...
              <div className="flex-1">
                <div className="text-sm font-bold text-text">{item.time}</div>
                <div className="text-xs text-muted mt-0.5">{item.why}</div>
              </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between">
        <Link to="/pillar/c/deload" className="flex items-center gap-2 text-sm text-muted hover:text-text transition-colors group">
          <span className="group-hover:-translate-x-1 transition-transform">←</span>
          Deload
        </Link>
        ...
        <Link to="/pillar/c/environment" className="flex items-center gap-2 text-sm text-muted hover:text-text transition-colors group justify-end">
          Thiết Kế Môi Trường
          <span className="group-hover:translate-x-1 transition-transform">→</span>
        </Link>
      </div>
