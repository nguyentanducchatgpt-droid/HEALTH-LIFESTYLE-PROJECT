  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-muted text-lg mb-6">5 câu hỏi phản tư để hiểu rõ hơn về tuần vừa qua và điều chỉnh cho tuần tiếp theo.</p>
        <div className="space-y-4">
          {WEEKLY_QUESTIONS.map(q => (
            <div key={q.id} className="rounded-2xl border border-border bg-surface p-4">
              <div className="flex items-start gap-3 mb-3">
                <span className="text-2xl">{q.icon}</span>
                <div className="text-lg font-medium text-text">{q.question}</div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                {q.options.map((opt, i) => (
                  <button key={i} onClick={() => setWeekAnswers(prev => ({ ...prev, [q.id]: i }))} className={`text-left text-base p-2 rounded-lg border transition-all ${weekAnswers[q.id] === i ? 'border-teal-400' : 'border-border hover:border-teal-500/30'}`} style={{ background: weekAnswers[q.id] === i ? `rgba(${RGB},0.1)` : undefined, color: weekAnswers[q.id] === i ? COLOR : undefined }}>
                    {opt}
                  </button>
                ))}
              </div>
