  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-text leading-tight animate-fade-in-up">Thiết Kế Môi Trường</h1>
          <span className="inline-block text-xs font-bold uppercase tracking-widest mt-3 mb-4 px-3 py-1 rounded-full border" style={{ color: COLOR, background: `rgba(${RGB},0.1)`, borderColor: `rgba(${RGB},0.2)` }}>C7 · Environment Design</span>
          <p className="text-muted text-base leading-relaxed max-w-2xl">Hành vi tốt không chỉ từ ý chí mạnh — mà từ môi trường được thiết kế để làm cho hành vi tốt dễ hơn. Mỗi thay đổi nhỏ trong không gian sống là một "thiết kế hành vi" vô hình nhưng mạnh mẽ.</p>
        </div>
      </div>
      ...
      <div className={`${ORBIT_CLASS} rounded-3xl p-[1.5px] mb-12`}>
        <div className="relative rounded-3xl overflow-hidden h-52 md:h-72">
          ...
          <div className="absolute bottom-4 left-6">
            <span className="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: COLOR, background: 'rgba(0,0,0,0.6)', borderColor: `rgba(${RGB},0.2)` }}>Môi trường quyết định hành vi · 3 không gian sống</span>
          </div>
        </div>
      </div>
      ...
      <RevealBlock className="mb-12">
        <div className="rounded-2xl p-5 border" style={{ borderColor: `rgba(${RGB},0.2)`, background: `rgba(${RGB},0.06)` }}>
          <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: COLOR }}>Nguyên Tắc Cốt Lõi</div>
      ...
      {/* 3 Environment zones */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>3 Không Gian Cần Thiết Kế</h2>
        <p className="text-muted text-sm mb-6">Tối ưu hóa từng giai đoạn trong ngày bắt đầu từ môi trường xung quanh bạn.</p>

        <div className="flex gap-2 mb-6 flex-wrap">
          {ENV_ZONES.map(z => (
            <button key={z.id} onClick={() => setActiveZone(z.id)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium transition-all border ${activeZone === z.id ? 'text-white' : 'text-muted border-border hover:border-rose-500/30'}`} style={{ background: activeZone === z.id ? z.color : undefined, borderColor: activeZone === z.id ? z.color : undefined }}>
        ...
        {zone && (
          <div className="rounded-2xl border p-5 md:p-6" style={{ borderColor: `${zone.color}30`, background: `${zone.color}06` }}>
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <div className="text-lg font-bold text-text">{zone.title}</div>
                <div className="text-xs font-bold uppercase tracking-widest mt-0.5" style={{ color: zone.color }}>{zone.subtitle}</div>
              </div>
            </div>
      ...
      {/* Quick wins table */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>8 Thay Đổi Nhanh, Tác Động Lớn</h2>
        <p className="text-muted text-sm mb-6">Bắt đầu với những gì dễ nhất — ngay hôm nay, không cần kế hoạch phức tạp.</p>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                {['Thay Đổi', 'Tác Động', 'Thời Gian', 'Chi Phí'].map(h => (
                  <th key={h} className="text-left py-3 pr-4 text-xs font-bold uppercase tracking-widest text-muted">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {QUICK_WINS.map((w, i) => (
                <tr key={i} className="border-b border-border/30 hover:bg-white/3 transition-colors">
                  ...
                  <td className="py-3 pr-4">
                    <span className="px-2 py-0.5 rounded-full text-xs font-bold" style={{ background: `${IMPACT_COLOR[w.impact]}20`, color: IMPACT_COLOR[w.impact] }}>{w.impact}</span>
                  </td>
      ...
      {/* 30-day challenge */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Thử Thách 30 Ngày</h2>
        <p className="text-muted text-sm mb-6">Thực hiện từng thay đổi theo tuần — không làm tất cả một lúc.</p>
        <div className="grid md:grid-cols-4 gap-3">
          {[
            ...
            <div key={w.week} className="rounded-xl border border-border bg-surface p-4">
              <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: COLOR }}>{w.week}</div>
              <div className="text-sm font-bold text-text mb-3">{w.focus}</div>
              <ul className="space-y-1">
                {w.changes.map((c, i) => (
                  <li key={i} className="flex items-start gap-2 text-xs text-muted"><span style={{ color: COLOR }}>→</span>{c}</li>
                ))}
              </ul>
            </div>
          ))}
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between">
        <Link to="/pillar/c/breathing" className="flex items-center gap-2 text-sm text-muted hover:text-text transition-colors group">
          <span className="group-hover:-translate-x-1 transition-transform">←</span>
          Kỹ Thuật Thở
        </Link>
        ...
        <Link to="/pillar/c/checklist" className="flex items-center gap-2 text-sm text-muted hover:text-text transition-colors group justify-end">
          Checklist Hằng Ngày
          <span className="group-hover:translate-x-1 transition-transform">→</span>
        </Link>
      </div>
