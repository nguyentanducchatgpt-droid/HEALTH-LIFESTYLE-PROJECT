...
export default function LifestyleNeatPage() {
  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      <Link to="/pillar/c" className="inline-flex items-center gap-2 text-muted text-sm mb-8 hover:text-emerald-400 transition-colors">
        ← Lối Sống Khỏe
      </Link>
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-text leading-tight">NEAT & Chống Ngồi Lâu</h1>
      ...
      {/* NEAT vs TEE */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>NEAT Chiếm Bao Nhiêu Trong Ngày?</h2>
        <p className="text-muted text-sm mb-6">Tổng năng lượng tiêu thụ trong ngày (TDEE) gồm 4 thành phần chính.</p>
        <div className="space-y-3">
          {NEAT_VS_TEE.map((item, i) => (
            <div key={i} className="flex items-center gap-4 p-3 rounded-xl" style={{ background: `${item.color}08`, border: `1px solid ${item.color}20` }}>
              ...
              <div>
                <div className="font-semibold text-text text-sm">{item.component}</div>
        ...
        <div className="mt-4 p-4 rounded-xl" style={{ background: `rgba(${RGB},0.07)`, border: `1px solid rgba(${RGB},0.2)` }}>
          <p className="text-sm text-muted"><strong style={{ color: COLOR }}>Điểm mấu chốt:</strong> Buổi tập gym chỉ chiếm 5–10% tổng năng lượng. NEAT chiếm 15–30%. Người năng động (đi lại nhiều) có NEAT cao hơn người ngồi nhiều tới 500–700 kcal/ngày.</p>
        </div>
      </RevealBlock>

      {/* NEAT activities */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>NEAT Tiêu Thụ Bao Nhiêu kcal?</h2>
        <p className="text-muted text-sm mb-6">Các hoạt động NEAT phổ biến và năng lượng tiêu thụ ước tính.</p>
          ...
          {NEAT_ACTIVITIES.map((a, i) => (
            <div key={i} className="flex items-center justify-between p-3 rounded-xl" style={{ background: `rgba(${RGB},0.05)`, border: `1px solid rgba(${RGB},0.1)` }}>
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-semibold text-text">{a.activity}</span>
              </div>
              <span className="text-sm font-bold tabular-nums" style={{ color: COLOR }}>{a.kcal} kcal</span>
            </div>
          ))}
        </div>
      </RevealBlock>
      ...
      <RevealBlock className="mb-12">
        <div className="p-6 rounded-2xl" style={{ background: `rgba(${RGB},0.08)`, border: `1px solid rgba(${RGB},0.25)` }}>
          <h2 className="text-xl font-bold mb-2" style={{ color: COLOR }}>⏱ Quy Tắc Đứng Dậy 2 Phút</h2>
      ...
      {/* Office hacks */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>6 Cách Tăng NEAT Cho Dân Văn Phòng</h2>
        <div className="grid gap-3">
          {OFFICE_HACKS.map((h, i) => (
            <div key={i} className="p-4 rounded-xl" style={{ background: `rgba(${RGB},0.04)`, border: `1px solid rgba(${RGB},0.08)` }}>
              <div className="font-semibold text-text text-sm mb-1">{h.hack}</div>
              <p className="text-muted text-xs leading-relaxed">{h.detail}</p>
            </div>
          ))}
        </div>
      </RevealBlock>
      ...
      {/* Step goals */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Mục Tiêu Bước Theo Cấp Độ</h2>
        <p className="text-muted text-sm mb-6">Không ép tất cả lên 10.000 bước. Tăng từ nền hiện tại, không nhảy vọt.</p>
        <div className="space-y-3">
          {STEP_GOALS.map((g, i) => (
            <div key={i} className="p-4 rounded-xl" style={{ background: `rgba(${RGB},0.05)`, border: `1px solid rgba(${RGB},0.1)` }}>
              <div className="flex justify-between items-start mb-1">
                <span className="font-semibold text-text text-sm">{g.profile}</span>
                <span className="text-xs font-bold" style={{ color: COLOR }}>{g.goal}</span>
              </div>
              <p className="text-xs text-muted">{g.tip}</p>
            </div>
          ))}
        </div>
      </RevealBlock>
      ...
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>NEAT Checklist Hằng Ngày</h2>
        ...
        <p className="text-xs text-muted">{checkCount}/5 — {checkCount >= 4 ? 'Xuất sắc! NEAT cao nhất trong ngày' : checkCount >= 3 ? 'Tốt' : 'Đang xây dựng thói quen'}</p>
      </RevealBlock>

      <div className="pt-6 border-t border-border flex items-center justify-between">
        <Link to="/pillar/c/morning" className="text-muted hover:text-emerald-400 transition-colors text-sm">← Routine Sáng</Link>
        <Link to="/pillar/c/recovery" className="text-sm font-semibold" style={{ color: COLOR }}>Phục Hồi →</Link>
      </div>
    </div>
  );
}
