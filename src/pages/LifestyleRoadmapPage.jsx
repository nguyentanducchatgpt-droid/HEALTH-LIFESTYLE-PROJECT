  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-text leading-tight animate-fade-in-up">Lộ Trình 12 Tuần</h1>
          <span className="inline-block text-xs font-bold uppercase tracking-widest mt-3 mb-4 px-3 py-1 rounded-full border" style={{ color: COLOR, background: `rgba(${RGB},0.1)`, borderColor: `rgba(${RGB},0.2)` }}>Lifestyle Roadmap</span>
          <p className="text-muted text-base leading-relaxed max-w-2xl">Hành trình từ "biết mình đang ở đâu" đến "sống với nhịp điệu bền vững". 12 tuần, 4 giai đoạn, từng bước rõ ràng — không vội vã, không áp lực.</p>
        </div>
      </div>
      ...
      <div className={`${ORBIT_CLASS} rounded-3xl p-[1.5px] mb-12`}>
        <div className="relative rounded-3xl overflow-hidden h-52 md:h-72">
          ...
          <div className="absolute bottom-4 left-6">
            <span className="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: COLOR, background: 'rgba(0,0,0,0.6)', borderColor: `rgba(${RGB},0.2)` }}>12 tuần · 4 giai đoạn · 8 module</span>
          </div>
        </div>
      </div>
      ...
      <RevealBlock className="mb-12">
        <div className="rounded-2xl p-5 border" style={{ borderColor: `rgba(${RGB},0.2)`, background: `rgba(${RGB},0.06)` }}>
          <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: COLOR }}>Triết Lý Của Lộ Trình Này</div>
      ...
      {/* 4 Phases */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>4 Giai Đoạn 12 Tuần</h2>
        <p className="text-muted text-sm mb-6">Mỗi giai đoạn xây trên nền của giai đoạn trước — đừng bỏ qua bước nào.</p>
        ...
        <div className="flex gap-1 mb-6 overflow-x-auto pb-2">
          {PHASES.map(p => (
            <button key={p.id} onClick={() => setOpenPhase(p.id)} className="flex-1 min-w-[100px] rounded-xl p-3 text-center transition-all border" style={{ background: openPhase === p.id ? `${p.color}15` : 'var(--color-surface)', borderColor: openPhase === p.id ? p.color : 'var(--color-border)' }}>
              ...
              <div className="text-xs font-bold" style={{ color: openPhase === p.id ? p.color : '#9ca3af' }}>Tuần {p.weeks}</div>
              <div className="text-xs text-muted mt-0.5 leading-tight">{p.subtitle}</div>
            </button>
          ))}
        </div>
        ...
        {active && (
          <div className="rounded-2xl border p-5 md:p-6" style={{ borderColor: `${active.color}30`, background: `${active.color}06` }}>
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <div className="text-xs font-bold uppercase tracking-widest" style={{ color: active.color }}>Tuần {active.weeks}</div>
                <div className="text-lg font-bold text-text">{active.title}</div>
                <div className="text-sm text-muted">{active.focus}</div>
              </div>
              <div className="ml-auto text-right">
                <div className="text-xs text-muted">Thời gian</div>
                <div className="text-sm font-bold" style={{ color: active.color }}>{active.daily}</div>
              </div>
            </div>

            <div className="grid md:grid-cols-3 gap-4">
              <div className="md:col-span-2">
                <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: active.color }}>Mục Tiêu Giai Đoạn</div>
                <ul className="space-y-2">
                  {active.goals.map((g, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-text">
                      <span className="w-5 h-5 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5" style={{ background: `${active.color}20`, color: active.color }}>{i + 1}</span>
                      {g}
                    </li>
                  ))}
                </ul>
              ...
              <div className="space-y-3">
                <div>
                  <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: active.color }}>Module Cần Học</div>
                  <div className="flex flex-wrap gap-1">
                    {active.modules.map(m => (
                      <span key={m} className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: `${active.color}15`, color: active.color }}>{m}</span>
                    ))}
                  </div>
                </div>
                <div>
                  <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: active.color }}>Đo Lường</div>
                  <ul className="space-y-1">
                    {active.metrics.map((m, i) => (
                      <li key={i} className="text-xs text-muted flex items-start gap-1"><span style={{ color: active.color }}>→</span>{m}</li>
                    ))}
                  </ul>
                </div>
              </div>
      ...
      {/* Daily rhythm */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Nhịp Ngày Lý Tưởng</h2>
        <p className="text-muted text-sm mb-6">Khung thời gian gợi ý — điều chỉnh theo lịch làm việc của bạn.</p>
        <div className="relative">
          <div className="absolute left-[68px] top-4 bottom-4 w-px bg-border" />
          <div className="space-y-3">
            {DAILY_RHYTHM.map((item, i) => (
              <div key={i} className="flex items-start gap-4">
                <div className="text-xs text-muted w-16 text-right shrink-0 pt-0.5 leading-tight">{item.time}</div>
      ...
      {/* All sub-pages */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Tất Cả Module Lối Sống</h2>
        <p className="text-muted text-sm mb-6">Khám phá theo trình tự lộ trình hoặc bắt đầu từ chủ đề bạn cần nhất.</p>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {SUB_PAGES.map(p => (
            <Link key={p.to} to={p.to} className="rounded-xl border border-border bg-surface p-4 hover:border-teal-500/20 transition-all group">
              ...
              <div className="text-sm font-bold text-text group-hover:text-white transition-colors">{p.title}</div>
              <div className="text-xs mt-1 font-bold" style={{ color: p.color }}>{p.week}</div>
            </Link>
          ))}
        </div>
      </RevealBlock>
      ...
      <RevealBlock className="mb-12">
        <blockquote className="rounded-2xl p-6 border-l-4 relative overflow-hidden" style={{ borderLeftColor: COLOR, background: `rgba(${RGB},0.05)` }}>
          ...
          <p className="text-lg font-medium text-text leading-relaxed italic">"Không cần sống hoàn hảo. Chỉ cần sống có nhịp, có hồi phục, có quay lại."</p>
          <cite className="text-xs text-muted mt-3 block">— Triết lý Lối sống khỏe</cite>
        </blockquote>
      </RevealBlock>

      <div className="h-px mb-8" style={{ background: 'linear-gradient(to right, transparent, var(--color-border), transparent)' }} />
      <div className="flex flex-col sm:flex-row gap-3 justify-between">
        <Link to="/pillar/c/assessment" className="flex items-center gap-2 text-sm text-muted hover:text-text transition-colors group">
          <span className="group-hover:-translate-x-1 transition-transform">←</span>
          Đánh Giá Lối Sống
        </Link>
        <Link to="/pillar/c" className="flex items-center gap-2 text-sm font-bold transition-colors justify-center" style={{ color: COLOR }}>
          Về Trang Chính →
        </Link>
      </div>
    </div>
