...
export default function LifestyleSleepPage() {
  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      <Link to="/pillar/c" className="inline-flex items-center gap-2 text-muted text-sm mb-8 hover:text-teal-400 transition-colors">
        ← Lối Sống Khỏe
      </Link>
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-text leading-tight">Khoa Học Giấc Ngủ</h1>
      ...
      {/* Why sleep matters */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Tại Sao Giấc Ngủ Quan Trọng?</h2>
        <p className="text-muted text-sm mb-6">Muốn khỏe bền, đừng chỉ tập thêm — hãy ngủ tốt hơn.</p>
        <div className="grid gap-3">
          {SLEEP_FACTS.map((f, i) => (
            <div key={i} className="flex gap-4 p-4 rounded-xl" style={{ background: `rgba(${RGB},0.05)`, border: `1px solid rgba(${RGB},0.1)` }}>
              <span className="text-2xl shrink-0">{f.icon}</span>
              <div>
                <div className="font-semibold text-text text-sm mb-1">{f.title}</div>
      ...
      {/* Sleep stages */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>4 Giai Đoạn Giấc Ngủ</h2>
        <p className="text-muted text-sm mb-6">Một chu kỳ ngủ hoàn chỉnh kéo dài ~90 phút và lặp lại 4–6 lần mỗi đêm.</p>
          ...
          {SLEEP_STAGES.map((st, i) => (
            <div key={i} className="p-4 rounded-xl border" style={{ borderColor: `${st.color}22`, background: `${st.color}08` }}>
              <div className="flex items-center justify-between mb-2">
                <span className="font-bold text-sm" style={{ color: st.color }}>{st.stage}</span>
                <span className="text-xs font-semibold tabular-nums" style={{ color: st.color }}>{st.time}</span>
              </div>
              <p className="text-muted text-xs leading-relaxed">{st.desc}</p>
            </div>
          ))}
        </div>
        <div className="mt-4 p-4 rounded-xl" style={{ background: `rgba(${RGB},0.07)`, border: `1px solid rgba(${RGB},0.15)` }}>
          <p className="text-xs text-muted"><strong style={{ color: COLOR }}>Lưu ý quan trọng:</strong> Giấc ngủ sâu (N3) nhiều nhất trong 3 giờ đầu đêm. REM nhiều nhất vào buổi sáng sớm. Ngủ muộn → mất giấc ngủ sâu; thức sớm → mất REM.</p>
        </div>
      </RevealBlock>

      {/* 4 steps hygiene */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Vệ Sinh Giấc Ngủ 4 Bước</h2>
        <p className="text-muted text-sm mb-6">Không ép ngủ hoàn hảo, mà xây môi trường để cơ thể dễ ngủ hơn.</p>
          ...
          {[
            ...
            <div key={i} className="p-5 rounded-2xl border" style={{ borderColor: `rgba(${RGB},0.15)`, background: `rgba(${RGB},0.04)` }}>
              <div className="flex items-center gap-3 mb-3">
                <span className="text-2xl font-black tabular-nums" style={{ color: `rgba(${RGB},0.3)` }}>{step.num}</span>
                <span className="text-xl">{step.icon}</span>
                <h3 className="font-bold text-text">{step.title}</h3>
              </div>
              <p className="text-muted text-sm leading-relaxed mb-3">{step.content}</p>
              <div className="text-xs font-semibold px-3 py-1.5 rounded-lg inline-block" style={{ color: COLOR, background: `rgba(${RGB},0.1)` }}>
                💡 {step.tip}
              </div>
            </div>
          ))}
      ...
      {/* 7-day reset */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Sửa Ngủ Muộn Trong 7 Ngày</h2>
        <p className="text-muted text-sm mb-6">Không kéo giờ ngủ sớm quá mạnh ngay. Mỗi 2–3 ngày kéo sớm 15–30 phút để cơ thể thích nghi.</p>
        <div className="space-y-3">
          {SEVEN_DAY_PLAN.map((day, i) => (
            <div key={i} className="flex gap-4 items-start p-3 rounded-xl" style={{ background: `rgba(${RGB},0.04)`, border: `1px solid rgba(${RGB},0.08)` }}>
              <div className="shrink-0 w-16 text-center">
                <div className="text-xs font-bold tabular-nums" style={{ color: COLOR }}>{day.day}</div>
              </div>
              <div>
                <div className="font-semibold text-text text-sm">{day.focus}</div>
                <div className="text-muted text-xs mt-0.5">{day.action}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Trouble cases */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Xử Lý Tình Huống Thường Gặp</h2>
        <p className="text-muted text-sm mb-6">3 kịch bản phổ biến nhất và cách xử lý thực tế.</p>
        <div className="space-y-3">
      ...
      {/* Daily checklist */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Checklist Ngủ Hằng Ngày</h2>
        <p className="text-muted text-sm mb-6">Không cần đạt 5/5 mỗi ngày. Đạt 3/5 là đã tốt cho người mới bắt đầu.</p>
        <div className="space-y-3">
          {[
            ...
            <label key={i} className="flex items-center gap-3 cursor-pointer group p-3 rounded-xl hover:bg-white/5 transition-colors">
              <div onClick={() => setChecks(p => ({ ...p, [i]: !p[i] }))}
                ...
                {checks[i] && <span className="text-black text-xs font-bold">✓</span>}
              </div>
        ...
        <p className="text-xs text-muted mt-2">{checkCount}/5 — {checkCount >= 4 ? '🌟 Xuất sắc!' : checkCount >= 3 ? '✅ Tốt!' : '⏳ Đang xây dựng'}</p>
      </RevealBlock>
      ...
      <div className="pt-6 border-t border-border flex items-center justify-between">
        <Link to="/pillar/c" className="text-muted hover:text-teal-400 transition-colors text-sm">← Lối Sống Khỏe</Link>
        <Link to="/pillar/c/sleep-routine" className="text-sm font-semibold" style={{ color: COLOR }}>Routine Trước Ngủ →</Link>
      </div>
    </div>
  );
}
