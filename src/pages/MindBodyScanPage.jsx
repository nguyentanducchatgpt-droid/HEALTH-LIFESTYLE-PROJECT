          ...
          {BODY_ZONES.map((z, i) => (
            <div key={z.id} className="rounded-xl border border-border overflow-hidden">
              <button onClick={() => setActiveZone(activeZone === i ? null : i)} className="w-full flex items-center gap-3 p-3 hover:bg-white/5 transition-colors text-left">
                ...
                <span className="text-muted text-base">{activeZone === i ? '▲' : '▼'}</span>
              </button>
              {activeZone === i && (
                <div className="px-4 pb-4 space-y-2">
                  <p className="text-lg text-text leading-relaxed border-t border-border pt-3">{z.instructions}</p>
                  <div className="text-base text-muted italic">💡 {z.tension}</div>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="grid md:grid-cols-2 gap-3">
          {[
            ...
            <div key={u.title} className="rounded-xl border border-border bg-surface p-4 hover:border-pink-500/20 transition-colors">
              ...
              <div className="text-lg font-bold text-text mb-1">{u.title}</div>
              <div className="text-base text-muted leading-relaxed">{u.desc}</div>
            </div>
          ))}
        </div>
      </RevealBlock>
