...
function SelfTalkTool() {
  ...
  return (
    <div className="rounded-2xl border p-5 space-y-4" style={{ borderColor: `${COLOR}33`, background: `${COLOR}08` }}>
      <p className="text-sm font-semibold" style={{ color: COLOR }}>Luyện Tập Tái Khung Suy Nghĩ Tiêu Cực</p>
      <div className="space-y-2">
        <div>
          <label className="text-xs text-muted mb-1 block">Suy nghĩ tiêu cực bạn hay nói với bản thân:</label>
          <textarea className="w-full rounded-xl border bg-bg text-text text-sm p-3 resize-none min-h-[60px] focus:outline-none focus:ring-2"
            style={{ borderColor: `${COLOR}30`, '--tw-ring-color': `${COLOR}60` }}
            placeholder="VD: Tôi thật tệ, tôi không có ý chí..."
            value={text} onChange={e => setText(e.target.value)} rows={2} />
        </div>
        <div>
          <label className="text-xs text-muted mb-1 block">Tái khung thành câu tích cực hơn:</label>
          <textarea className="w-full rounded-xl border bg-bg text-text text-sm p-3 resize-none min-h-[60px] focus:outline-none focus:ring-2"
            style={{ borderColor: `${COLOR}30`, '--tw-ring-color': `${COLOR}60` }}
      ...
      <div className="border-t border-border pt-4">
        <p className="text-xs text-muted mb-3">Ví dụ thực tế ({exIdx + 1}/{examples.length}):</p>
        <div className="space-y-2 rounded-xl bg-surface border border-border p-4">
          <div className="flex gap-2 items-start"><span className="text-xs font-bold text-red-400 shrink-0">❌</span><p className="text-sm text-muted">{examples[exIdx][0]}</p></div>
          <div className="flex gap-2 items-start"><span className="text-xs font-bold shrink-0" style={{ color: COLOR }}>✓</span><p className="text-sm text-text">{examples[exIdx][1]}</p></div>
        </div>
  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-text leading-tight animate-fade-in-up">Kỷ Luật Mềm — Gentle Discipline</h1>
          <span className="inline-block text-xs font-bold uppercase tracking-widest mt-3 mb-4 px-3 py-1 rounded-full border" style={{ color: COLOR, background: `${COLOR}15`, borderColor: `${COLOR}30` }}>D6 · Tự Trắc Ẩn &amp; Kỷ Luật</span>
          <p className="text-muted text-base leading-relaxed max-w-2xl">Kỷ luật bền vững không đến từ sự khắc nghiệt với bản thân, mà từ sự tử tế và hiểu mình. Gentle discipline là nghệ thuật kiên định mà không tự trừng phạt.</p>
        </div>
      </div>
      ...
      <div className={`${ORBIT_CLASS} rounded-3xl p-[1.5px] mb-12`}>
        <div className="relative rounded-3xl overflow-hidden h-52 md:h-72">
          ...
          <div className="absolute bottom-4 left-6">
            <span className="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: COLOR, background: 'rgba(10,10,10,0.6)', borderColor: `${COLOR}30` }}>6 Nguyên Tắc · Thực Hành Mỗi Ngày</span>
          </div>
        </div>
      </div>
      ...
      <RevealBlock className="mb-10">
        <div className="rounded-2xl border p-5 md:p-6" style={{ borderColor: `${COLOR}25`, background: `${COLOR}07` }}>
          <p className="text-base text-text leading-relaxed italic">"Sự tử tế với bản thân không phải sự yếu đuối — đó là nền tảng của kỷ luật bền vững. Người tử tế với bản thân khi thất bại thường tiếp tục cố gắng lâu dài hơn."</p>
          <p className="text-xs mt-3" style={{ color: COLOR }}>— Kristin Neff, Self-Compassion Research</p>
        </div>
      </RevealBlock>

      <RevealBlock className="mb-10">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>6 Nguyên Tắc Kỷ Luật Mềm</h2>
        <p className="text-muted text-sm mb-6">Xây dựng kỷ luật dựa trên sức mạnh nội tâm, không phải nỗi sợ</p>
      ...
      <RevealBlock className="mb-10">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Kỷ Luật Mềm vs Khắc Nghiệt</h2>
        <p className="text-muted text-sm mb-6">Thay thế ngôn ngữ tự phán xét bằng tự trắc ẩn</p>
          ...
          {VS.map((v, i) => (
            <div key={i} className="rounded-2xl border border-border bg-surface p-4 grid grid-cols-2 gap-4">
              <div className="flex items-start gap-2">
                ...
                <p className="text-sm text-muted">{v.harsh}</p>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-sm shrink-0 mt-0.5" style={{ color: COLOR }}>✓</span>
                <p className="text-sm text-text">{v.soft}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <RevealBlock className="mb-10">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Thực Hành Tái Khung</h2>
        <p className="text-muted text-sm mb-6">Luyện tập nói chuyện tử tế với bản thân</p>
        <SelfTalkTool />
      </RevealBlock>

      <RevealBlock className="mb-10">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Khung Thực Hành 3 Buổi</h2>
        <p className="text-muted text-sm mb-6">Micro-habits trong ngày để xây kỷ luật bền vững</p>
