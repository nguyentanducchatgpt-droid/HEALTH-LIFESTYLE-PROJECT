...
function MeditationTimer({ color }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-bg p-5 flex flex-col items-center gap-4 max-w-xs mx-auto">
      <div className="text-xs font-bold uppercase tracking-widest text-muted">Meditation Timer</div>
      {!running && !done && (
        <div className="flex gap-2">
          {[3, 5, 10].map(d => (
            <button key={d} onClick={() => setDuration(d)} className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all border ${duration === d ? 'text-white' : 'text-muted border-border'}`} style={{ background: duration === d ? color : undefined, borderColor: duration === d ? color : undefined }}>
              {d} ph
            </button>
          ))}
        </div>
      ...
      <div className="relative w-32 h-32 flex items-center justify-center">
        ...
        <div className="text-center">
          {done ? (
            ...
            <>
              <div className="text-2xl font-bold text-text">{String(mins).padStart(2, '0')}:{String(secs).padStart(2, '0')}</div>
              <div className="text-xs text-muted">{running ? 'thiền...' : `${duration} phút`}</div>
            </>
          )}
        </div>
      </div>
      {done ? (
        <div className="text-sm font-bold text-center" style={{ color }}>Hoàn thành! 🧘</div>
      ) : (
        <button onClick={running ? stop : start} className="px-5 py-2 rounded-full text-sm font-bold" style={{ background: running ? 'rgba(239,68,68,0.15)' : `rgba(${RGB},0.15)`, color: running ? '#ef4444' : color, border: `1px solid ${running ? 'rgba(239,68,68,0.3)' : `rgba(${RGB},0.3)`}` }}>
  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-text leading-tight animate-fade-in-up">Thiền Ngắn & Chánh Niệm</h1>
          <span className="inline-block text-xs font-bold uppercase tracking-widest mt-3 mb-4 px-3 py-1 rounded-full border" style={{ color: COLOR, background: `rgba(${RGB},0.1)`, borderColor: `rgba(${RGB},0.2)` }}>D3 · Thiền & Chánh Niệm</span>
          <p className="text-muted text-base leading-relaxed max-w-2xl">Thiền không phải là "không suy nghĩ". Thiền là nhận ra mình đang bị cuốn đi và quay lại nhẹ nhàng. Mỗi lần quay lại là một lần tập — không cần hoàn hảo.</p>
        </div>
      </div>
      ...
      <div className={`${ORBIT_CLASS} rounded-3xl p-[1.5px] mb-12`}>
        <div className="relative rounded-3xl overflow-hidden h-52 md:h-72">
          ...
          <div className="absolute bottom-4 left-6">
            <span className="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: COLOR, background: 'rgba(0,0,0,0.6)', borderColor: `rgba(${RGB},0.2)` }}>4 kiểu thiền · Timer tương tác · Từ 3 phút</span>
          </div>
        </div>
      </div>
      ...
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Hiểu Đúng Về Thiền</h2>
        <p className="text-muted text-sm mb-6">4 hiểu lầm phổ biến khiến người mới bỏ cuộc trước khi bắt đầu.</p>
        <div className="space-y-3">
          {MYTHS.map((m, i) => (
            <div key={i} className="rounded-xl border border-border bg-surface p-4 grid md:grid-cols-2 gap-3">
              <div className="flex items-start gap-2"><span className="text-red-400 text-sm shrink-0">✗</span><div className="text-sm text-muted line-through">{m.myth}</div></div>
              <div className="flex items-start gap-2"><span className="text-sm shrink-0" style={{ color: COLOR }}>✓</span><div className="text-sm text-text">{m.truth}</div></div>
            </div>
          ))}
        </div>
      </RevealBlock>
      ...
      {/* Practice types */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>4 Kiểu Thiền Cốt Lõi</h2>
        <p className="text-muted text-sm mb-6">Chọn kiểu phù hợp với hoàn cảnh và sở thích của bạn.</p>
        <div className="flex gap-2 flex-wrap mb-6">
          {PRACTICE_TYPES.map(p => (
            <button key={p.id} onClick={() => setActive(p.id)} className={`flex items-center gap-2 px-3 py-2 rounded-full text-xs font-medium transition-all border ${active === p.id ? 'text-white' : 'text-muted border-border'}`} style={{ background: active === p.id ? p.color : undefined, borderColor: active === p.id ? p.color : undefined }}>
              {p.icon} {p.title}
            </button>
          ))}
        </div>
        {practice && (
          <div className="rounded-2xl border p-5 md:p-6" style={{ borderColor: `${practice.color}30`, background: `${practice.color}06` }}>
            <div className="flex items-start gap-4 mb-5">
              ...
              <div>
                <div className="text-lg font-bold text-text">{practice.title}</div>
                <div className="text-xs font-bold uppercase tracking-widest mt-0.5" style={{ color: practice.color }}>{practice.subtitle}</div>
              </div>
            </div>
            <div className="space-y-3 mb-5">
              {practice.steps.map((s, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-xl border border-border bg-bg">
                  <span className="text-xs font-bold shrink-0 mt-0.5 min-w-[55px]" style={{ color: practice.color }}>{s.time}</span>
                  <span className="text-sm text-text leading-relaxed">{s.desc}</span>
                </div>
              ))}
            </div>
            <div className="rounded-xl p-3 text-xs" style={{ background: `${practice.color}10` }}>
      ...
      {/* Timer */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Meditation Timer</h2>
        <p className="text-muted text-sm mb-6">Thực hành ngay — chọn thời lượng và bắt đầu thiền.</p>
        <MeditationTimer color={COLOR} />
      </RevealBlock>

      {/* Week plan */}
      <RevealBlock className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-1" style={{ color: COLOR }}>Lộ Trình Tăng Dần</h2>
        <p className="text-muted text-sm mb-6">Bắt đầu từ 3 phút, tăng dần trong 7 ngày.</p>
        <div className="grid grid-cols-4 md:grid-cols-7 gap-2">
          {[
            ...
            <div key={d.day} className="rounded-xl border border-border bg-surface p-3 text-center">
              <div className="text-xs font-bold" style={{ color: COLOR }}>{d.day}</div>
              <div className="text-lg font-bold text-text mt-1">{d.mins}</div>
              <div className="text-xs text-muted">phút</div>
            </div>
          ))}
        </div>
      </RevealBlock>
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between">
        <Link to="/pillar/d/breathing" className="flex items-center gap-2 text-sm text-muted hover:text-text transition-colors group">
          <span className="group-hover:-translate-x-1 transition-transform">←</span>
          Kỹ Thuật Thở
        </Link>
        <Link to="/pillar/d" className="text-sm text-muted hover:text-purple-400 transition-colors text-center">Tâm Trí An Nhiên →</Link>
        <Link to="/pillar/d/body-scan" className="flex items-center gap-2 text-sm text-muted hover:text-text transition-colors group justify-end">
          Body Scan
          <span className="group-hover:translate-x-1 transition-transform">→</span>
        </Link>
      </div>
