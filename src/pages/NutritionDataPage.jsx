    ...
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-10">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          ...
          <span className="inline-block text-base font-bold uppercase tracking-widest text-cyan-400 mt-3 mb-4 px-3 py-1 bg-cyan-400/10 border border-cyan-400/20 rounded-full">
      ...
      <div className="nd-orbit-ring rounded-3xl p-[1.5px] mb-12">
        <div className="relative rounded-3xl overflow-hidden h-52 md:h-72">
          ...
          <span className="absolute bottom-4 left-6 text-cyan-400 text-base font-bold uppercase tracking-widest bg-bg/60 px-3 py-1 rounded-full border border-cyan-400/20">
      ...
      <RevealBlock className="mb-10">
        <div className="rounded-2xl border border-cyan-400/20 bg-surface p-5">
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <span className="text-base font-bold uppercase tracking-widest text-cyan-400">Chỉ số của bạn</span>
            <span className="text-base text-muted">({s.w}kg · {s.h}cm · {s.a}t · {s.s === 'male' ? 'Nam' : 'Nữ'} · {s.act.label} · {s.gm.label})</span>
          </div>
            ...
            {[
              ...
              <div key={i} className="group/stat relative">
                ...
                <div className="rounded-xl bg-bg border border-border p-3 text-center cursor-default">
                  ...
                  <div className="text-base text-muted mt-0.5">{item.label}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-4 text-base text-muted mt-2">
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-muted text-lg mb-6">4 cơ sở dữ liệu liên kết — click để xem chi tiết từng database.</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {DATABASES.map(db => (
            ...
            >
              ...
              <div className="text-base font-bold" style={{ color: db.color }}>{db.label}</div>
              <div className="text-base text-muted mt-1 leading-snug">{db.fields.length} fields</div>
            </button>
          ))}
        </div>

        <div className="rounded-2xl border border-border bg-surface p-5 mb-6">
          <h3 className="text-lg font-bold text-muted uppercase tracking-widest mb-4">Data Flow Diagram</h3>
          <DataFlowDiagram />
        </div>
      </RevealBlock>
      ...
      <RevealBlock className="mb-12">
        <div className="rounded-2xl border overflow-hidden" style={{ borderColor: `${activeDbData.color}40` }}>
          <div className="relative h-32 overflow-hidden">
            ...
            <div className="absolute inset-0 flex items-center px-6 gap-4">
              ...
              <div>
                ...
                <div className="text-base text-muted">{activeDbData.notion} · {activeDbData.gsheet}</div>
              </div>
              <div className="ml-auto flex gap-2">
                <span className="text-base px-2 py-1 rounded-full border font-medium" style={{ color: activeDbData.color, borderColor: `${activeDbData.color}40`, background: `${activeDbData.color}15` }}>
                  Notion
                </span>
                <span className="text-base px-2 py-1 rounded-full border border-border text-muted bg-surface">
                  Google Sheet
                </span>
              </div>
            </div>
          ...
          <div className="p-5">
            <p className="text-lg text-muted mb-5">{activeDbData.desc}</p>

            <div className="overflow-x-auto">
              <table className="w-full text-lg">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left text-base text-muted font-semibold uppercase tracking-wider pb-3 pr-4">Field</th>
                    <th className="text-left text-base text-muted font-semibold uppercase tracking-wider pb-3 pr-4">Type</th>
                    <th className="text-left text-base text-muted font-semibold uppercase tracking-wider pb-3 pr-4">Description</th>
                    <th className="text-left text-base text-muted font-semibold uppercase tracking-wider pb-3">Example</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={i} className="border-b border-border/50 hover:bg-white/[0.02] transition-colors">
                      <td className="py-2.5 pr-4">
                        <div className="flex items-center gap-2">
                          <span>{f.icon}</span>
                          <span className="font-mono text-base text-text font-medium">{f.name}</span>
                        </div>
                      </td>
                      <td className="py-2.5 pr-4">
                        <span className={`text-base px-2 py-0.5 rounded border font-medium ${TYPE_BADGE[f.type] || 'bg-surface text-muted border-border'}`}>
                          {f.type}
                        </span>
                      </td>
                      <td className="py-2.5 pr-4 text-muted text-base leading-relaxed">
                        {f.desc}
                        {f.formula && (
                          <div className="font-mono text-orange-400 text-[10px] mt-0.5 opacity-70">{f.formula}</div>
                        )}
                      </td>
                      <td className="py-2.5 text-base text-text/60 font-mono">{f.example}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-muted text-lg mb-6">Copy công thức vào Google Sheet — thay ô tham chiếu theo layout của bạn.</p>
          ...
          {GSHEET_FORMULAS.map((f, i) => (
            <div key={i} className="rounded-xl border border-border bg-surface p-4 hover:border-opacity-60 transition-all"
              ...
              <div className="flex items-start justify-between gap-3 mb-2">
                <div>
                  <div className="text-lg font-bold text-text">{f.label}</div>
                  <div className="text-base text-muted">{f.desc}</div>
                </div>
                <button
                  onClick={() => handleCopy(f.formula, i)}
                  className="shrink-0 text-base px-2.5 py-1 rounded-lg border transition-all duration-150"
                  style={{
              ...
              <div className="font-mono text-base bg-bg rounded-lg px-3 py-2 border border-border text-text/80 mb-2">
                {f.formula}
              </div>
        ...
        <div className="mt-5 rounded-xl border border-cyan-400/20 bg-cyan-400/5 p-4">
          <div className="text-base font-bold text-cyan-400 uppercase tracking-widest mb-2">Ví dụ với số liệu của bạn</div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-lg">
            <div><span className="text-muted text-base">BMR:</span> <span className="text-cyan-400 font-bold">{s.bmr.toLocaleString()} kcal</span></div>
            <div><span className="text-muted text-base">TDEE:</span> <span className="text-cyan-400 font-bold">{s.tdee.toLocaleString()} kcal</span></div>
            <div><span className="text-muted text-base">Mục tiêu:</span> <span className="text-cyan-400 font-bold">{s.targetKcal.toLocaleString()} kcal</span></div>
            <div><span className="text-muted text-base">BMI:</span> <span className="text-cyan-400 font-bold">{s.bmi}</span></div>
          </div>
        </div>
      </RevealBlock>
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-muted text-lg mb-6">Dán vào property Formula trong Notion database.</p>

        <div className="space-y-3">
          {NOTION_FORMULAS.map((f, i) => (
            <div key={i} className="rounded-xl border border-border bg-surface p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-lg font-bold text-text">{f.prop}</span>
                <button
                  onClick={() => handleCopyNotion(f.formula, i)}
                  className="text-base px-2.5 py-1 rounded-lg border border-lime-500/30 text-lime-400 bg-lime-500/10 transition-all duration-150 hover:bg-lime-500/20"
                >
                  {copiedNotion === i ? '✓ Copied' : 'Copy'}
                </button>
              </div>
              <pre className="font-mono text-base bg-bg rounded-lg px-3 py-2.5 border border-border text-lime-300/80 whitespace-pre-wrap break-all">
                {f.formula}
              </pre>
            </div>
          ))}
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-muted text-lg mb-6">15 thực phẩm Việt Nam phổ biến — dinh dưỡng trên khẩu phần chuẩn.</p>
        ...
        <div className="flex gap-2 mb-5 flex-wrap">
          {cats.map(cat => (
            <button
              ...
              className="text-base px-3 py-1.5 rounded-full border font-medium transition-all duration-150"
              style={{
          ...
          {filteredFoods.map((food, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                ...
                <div>
                  <div className="text-lg font-semibold text-text">{food.name}</div>
                  <div className="text-base text-muted">{food.portion}</div>
                </div>
              </div>
        ...
        <div className="rounded-2xl border border-lime-500/30 bg-surface p-5">
          <h3 className="text-lg font-bold uppercase tracking-widest text-lime-400 mb-4">Máy Tính Khẩu Phần</h3>
          <div className="flex flex-col md:flex-row gap-5 items-start">
            <div className="flex-1">
              <label className="text-base text-muted mb-1.5 block">Chọn thực phẩm</label>
              <select
                className="w-full bg-bg border border-border rounded-xl px-3 py-2.5 text-lg text-text focus:outline-none focus:border-lime-500/50"
            ...
            <div className="w-full md:w-44">
              <label className="text-base text-muted mb-1.5 block">Mục tiêu protein (g)</label>
              <input
                type="number"
                min={1}
                max={300}
            ...
            <div className="w-full md:w-56 rounded-xl border border-lime-500/30 bg-lime-500/5 p-4">
              <div className="text-base text-muted mb-1">Cần ăn</div>
              <div className="text-3xl font-bold text-lime-400">{gramsNeeded}g</div>
              <div className="text-base text-muted">{selectedFood.name}</div>
              <div className="text-base text-muted mt-1">
                = {kcalResult} kcal · {targetProtein}g protein
              </div>
            </div>
          </div>
          <div className="mt-4 text-base text-muted font-mono bg-bg rounded-lg px-3 py-2 border border-border">
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-muted text-lg mb-6">Tích các tiêu chí bạn đã đạt hôm nay để xem điểm tuân thủ.</p>
        ...
        <div className="rounded-2xl border border-cyan-400/20 bg-surface p-6">
          <div className="flex flex-col md:flex-row gap-8 items-start">
              ...
              {SCORE_CRITERIA.map(c => (
                <label key={c.key} className="flex items-start gap-3 cursor-pointer group/crit">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span>{c.icon}</span>
                      <span className={`text-lg font-medium transition-colors ${scoreChecks[c.key] ? 'text-text' : 'text-muted'}`}>{c.label}</span>
                      <span className="text-base font-bold rounded-full px-1.5 py-0.5" style={{ color: c.color, background: `${c.color}15` }}>+{c.points}</span>
                    </div>
                    <div className="text-base text-muted mt-0.5">{c.tip}</div>
                  </div>
                </label>
              ))}
            </div>
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="text-lg font-bold" style={{ color: scoreLevelColor }}>{scoreLevel}</div>
              <div className="text-base text-muted text-center">
                {score === 100 ? 'Hoàn hảo! Duy trì nhé.' : score >= 80 ? 'Rất tốt hôm nay.' : score >= 50 ? 'Hãy cải thiện thêm.' : 'Cần chú ý hơn.'}
              </div>
              <div className="text-base text-muted text-center mt-1">
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-muted text-lg mb-6">Mẫu nhật ký một ngày điển hình.</p>
        ...
        <div className="rounded-2xl border border-cyan-400/20 bg-surface overflow-hidden">
          <div className="px-5 py-3 border-b border-border bg-bg">
            <div className="flex items-center gap-2">
              ...
              <span className="text-base text-muted ml-2">Daily Nutrition Log — 2025-01-15</span>
            </div>
          </div>
          <div className="overflow-x-auto">
