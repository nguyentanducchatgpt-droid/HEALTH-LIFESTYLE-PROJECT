...
function FormulaCard({ label, formula, result, color = GREEN }) {
  return (
    <div className="rounded-xl p-4 border" style={{ background: `${color}08`, borderColor: `${color}25` }}>
      ...
      <div className="font-mono text-sm font-semibold text-text mb-1">{formula}</div>
      {result && <div className="font-mono text-xs text-muted mt-1">{result}</div>}
    </div>
  );
}
...
export default function NutritionFormulaPage() {
  ...
  const stepContent = [
    ...
    <div key="s0" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: GREEN }}>Nhập thông số cá nhân</div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input type="number" value={localInputs.weight} onChange={e => setLocalInputs(p => ({ ...p, weight: e.target.value }))}
            className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm text-text focus:outline-none focus:border-green-500" />
        </div>
        <div>
      ...
      <div className="grid grid-cols-2 gap-2 pt-1">
        {[{ label: 'BMI', val: s.bmi }, { label: 'BMR', val: `${s.bmr.toLocaleString()} kcal` }, { label: 'TDEE', val: `${s.tdee.toLocaleString()} kcal` }, { label: 'Mục tiêu', val: `${s.targetKcal.toLocaleString()} kcal` }].map((item, i) => (
          <div key={i} className="rounded-xl p-3 border text-center" style={{ background: `${GREEN}08`, borderColor: `${GREEN}25` }}>
            ...
            <div className="font-bold text-sm nf-count" style={{ color: GREEN }}>{item.val}</div>
          </div>
        ))}
      </div>
    </div>,
    ...
    <div key="s1" className="space-y-4">
      ...
      <div className="flex flex-col items-center gap-2">
        <BMIGauge bmi={s.bmi} />
        <div className="text-2xl font-bold nf-count" style={{ color: bmiLabel(s.bmi).color }}>{s.bmi}</div>
        <div className="text-xs font-semibold px-3 py-1 rounded-full border" style={{ color: bmiLabel(s.bmi).color, borderColor: `${bmiLabel(s.bmi).color}40`, background: `${bmiLabel(s.bmi).color}12` }}>{bmiLabel(s.bmi).label}</div>
      </div>
      <div className="rounded-xl border overflow-hidden" style={{ borderColor: '#2a2a2a' }}>
        ...
        {[
          ...
          <div key={i} className="grid grid-cols-3 px-3 py-2 text-xs border-t" style={{ borderColor: '#1a1a1a', background: i % 2 === 0 ? 'transparent' : '#0d0d0d' }}>
      ...
      <div className="rounded-xl p-3 border text-xs text-muted" style={{ borderColor: '#2a2a2a', background: '#0d0d0d' }}>
    ...
    <div key="s2" className="space-y-4">
      ...
      <div className="rounded-xl p-4 border" style={{ background: `${GREEN}08`, borderColor: `${GREEN}25` }}>
        ...
        <div className="font-mono text-xl font-bold mt-2" style={{ color: GREEN }}>= {s.bmr.toLocaleString()} kcal/ngày</div>
      </div>
      <div className="rounded-xl p-3 border text-xs text-muted space-y-1" style={{ borderColor: '#2a2a2a', background: '#0d0d0d' }}>
    ...
    <div key="s3" className="space-y-4">
      ...
      <div className="rounded-xl border overflow-hidden" style={{ borderColor: '#2a2a2a' }}>
        ...
        {ACTIVITY_LEVELS.map((al, i) => (
          <div key={i} className="grid grid-cols-3 px-3 py-2.5 text-xs border-t transition-colors"
          ...
          return (
            <div key={g.key} className="rounded-xl p-3 border flex items-center gap-3 transition-all"
              ...
              <div className="text-right">
                <div className="font-mono text-xs text-muted">{s.tdee} {g.delta >= 0 ? '+' : ''}{g.delta}</div>
                <div className="font-bold text-sm" style={{ color: g.color }}>{kcal.toLocaleString()} kcal</div>
              </div>
            </div>
          );
        })}
    ...
    <div key="s5" className="space-y-4">
      ...
      <div className="rounded-xl border overflow-hidden" style={{ borderColor: '#2a2a2a' }}>
        ...
        {[
          ...
          <div key={i} className="grid grid-cols-2 px-3 py-2 text-xs border-t" style={{ borderColor: '#1a1a1a', background: i % 2 === 0 ? 'transparent' : '#0d0d0d' }}>
            <span style={{ color: row.color }}>{row.label}</span>
            <span className="font-mono font-bold" style={{ color: row.color }}>{row.range}</span>
          </div>
        ))}
      ...
      <div className="rounded-xl p-3 border" style={{ background: `${GREEN}08`, borderColor: `${GREEN}25` }}>
        ...
        <div className="font-mono text-sm font-bold" style={{ color: GREEN }}>{s.proteinG}g ÷ 3 bữa ≈ {s.perMealProtein}g/bữa</div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={i} className="flex items-center gap-2 rounded-lg p-2 border text-xs" style={{ borderColor: '#1a1a1a', background: '#0d0d0d' }}>
    ...
    <div key="s6" className="space-y-4">
      ...
      <div className="rounded-xl p-3 border text-xs" style={{ background: '#ef444410', borderColor: '#ef444430' }}>
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={i} className="rounded-xl p-3 border text-center" style={{ background: `${item.color}0d`, borderColor: `${item.color}30` }}>
            ...
            <div className="font-mono font-bold text-sm" style={{ color: item.color }}>{Math.round(s.carbG * item.mult)}g</div>
    ...
    <div key="s8" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: LIME }}>Chọn mô hình bữa ăn</div>
      ...
      <div className="rounded-xl p-3 border" style={{ background: '#0d0d0d', borderColor: '#2a2a2a' }}>
        ...
        <div className="font-mono text-xs text-text">Kcal mỗi bữa = Tổng × Tỉ lệ%</div>
    ...
    <div key="s9" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: '#f59e0b' }}>Calorie Cycling — Theo Ngày Tập</div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={i} className="rounded-xl p-2.5 border text-center" style={{ background: `${item.color}0d`, borderColor: `${item.color}30` }}>
            ...
            <div className="font-bold text-sm" style={{ color: item.color }}>{item.g}g</div>
          </div>
        ))}
      </div>
      <div className="rounded-xl p-3 border space-y-2" style={{ background: '#0d0d0d', borderColor: '#2a2a2a' }}>
        ...
        <div className="font-mono text-xs text-text">Carb = 0.5–1g/kg = <span style={{ color: GREEN }}>{s.preCarb}g</span></div>
        <div className="font-mono text-xs text-text">Protein = 0.2–0.3g/kg = <span style={{ color: GREEN }}>{s.preProtein}g</span></div>
        <div className="border-t mt-2 pt-2" style={{ borderColor: '#2a2a2a' }}>
          ...
          <div className="font-mono text-xs text-text">Protein = 0.25–0.4g/kg = <span style={{ color: LIME }}>{s.postProtein}g</span></div>
          <div className="font-mono text-xs text-text">Carb = 0.5–1g/kg = <span style={{ color: LIME }}>{s.postCarb}g</span></div>
        </div>
      </div>
    </div>,
    ...
    <div key="s10" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: '#f59e0b' }}>Protein & Fat cố định · Chỉ Carb thay đổi</div>
    ...
    <div key="s11" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: '#06b6d4' }}>Không cần cân — nhìn đĩa là biết</div>
      ...
      <div className="grid gap-2">
        {[
          ...
          <div key={i} className="flex items-center gap-3 rounded-xl px-4 py-2.5 border" style={{ background: `${item.color}08`, borderColor: `${item.color}25` }}>
            ...
            <div className="flex-1">
              <div className="font-bold text-xs" style={{ color: item.color }}>{item.label}</div>
    ...
    <div key="s12" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: '#f59e0b' }}>Xung quanh buổi tập</div>
      <div className="grid gap-3">
        <div className="rounded-xl p-4 border" style={{ background: 'rgba(245,158,11,0.06)', borderColor: 'rgba(245,158,11,0.25)' }}>
          <div className="flex items-center gap-2 mb-3"><span className="text-lg">⚡</span><div className="font-bold text-sm text-text">Pre-Workout <span className="text-[10px] text-muted font-normal">(1–2h trước tập)</span></div></div>
        ...
        <div className="rounded-xl p-4 border" style={{ background: 'rgba(34,197,94,0.06)', borderColor: 'rgba(34,197,94,0.25)' }}>
          <div className="flex items-center gap-2 mb-3"><span className="text-lg">💥</span><div className="font-bold text-sm text-text">Post-Workout <span className="text-[10px] text-muted font-normal">(30–60 phút sau tập)</span></div></div>
    ...
    <div key="s13" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: '#06b6d4' }}>Tập &gt;60 phút</div>
      <div className="grid gap-3">
        {[
          ...
          <div key={i} className="rounded-xl p-4 border" style={{ background: `${item.color}08`, borderColor: `${item.color}25` }}>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <div className="font-mono text-xs" style={{ color: item.color }}>{item.formula}</div>
              </div>
            </div>
            <div className="text-[10px] text-muted">{item.note}</div>
          </div>
    ...
    <div key="s14" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: '#06b6d4' }}>3 chỉ tiêu thường bị bỏ qua</div>
      <div className="grid gap-3">
        ...
        <div className="rounded-xl p-3 border text-[10px]" style={{ background: 'rgba(132,204,22,0.06)', borderColor: 'rgba(132,204,22,0.25)' }}>
          ...
          <div className="font-mono text-xs text-text mb-1">3–5 phần rau / ngày · 1 phần = 80–100g</div>
    ...
    <div key="s15" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: '#a855f7' }}>Không cần cân — dùng tay</div>
      <div className="grid grid-cols-2 gap-2">
        {[
    ...
    <div key="s16" className="space-y-4">
      <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color: GREEN }}>Ghép thực phẩm thực tế</div>
      ...
      {(() => {
        ...
        return (
          <div className="rounded-2xl border overflow-hidden" style={{ borderColor: '#2a2a2a' }}>
            <div className="px-4 py-2.5 font-bold text-xs" style={{ background: '#0d0d0d', color: LIME }}>
            ...
            {mealsData.map((meal, mi) => (
              <div key={mi} className="border-t" style={{ borderColor: '#1e1e1e' }}>
                <div className="px-4 py-2 flex items-center justify-between" style={{ background: `${meal.color}08` }}>
                  <div><span className="font-bold text-xs" style={{ color: meal.color }}>{meal.name}</span><span className="text-muted text-[9px] ml-2">{meal.time} · {meal.pct}%</span></div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-10">
      <Link to="/pillar/b" className="inline-flex items-center gap-2 text-xs text-muted hover:text-lime-400 transition-colors mb-8 group">
      ...
      <RevealBlock>
        <div className="flex items-start gap-6 mb-10 relative">
          ...
          <div className="w-20 h-20 rounded-3xl text-5xl flex items-center justify-center shrink-0 nf-float border" style={{ background: 'rgba(132,204,22,0.08)', borderColor: 'rgba(132,204,22,0.2)' }}>
            🧮
          </div>
          <div className="flex-1 min-w-0">
            <h1 className="text-4xl md:text-5xl font-bold leading-tight nf-fade-in-up">
              <span className="nf-shimmer">Công Thức Tính Meal Plan</span>
            </h1>
            <div className="inline-block text-xs font-bold uppercase tracking-widest mt-3 mb-4 px-3 py-1 rounded-full border" style={{ color: LIME, background: 'rgba(132,204,22,0.1)', borderColor: 'rgba(132,204,22,0.2)' }}>
        ...
        <div className="nf-orbit-ring rounded-3xl p-[1.5px] mb-12">
          <div className="relative rounded-3xl overflow-hidden h-52 md:h-72">
            ...
            <div className="absolute bottom-4 left-6">
              <span className="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: LIME, background: 'rgba(10,10,10,0.6)', borderColor: 'rgba(132,204,22,0.2)' }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-12">
          {[
            ...
            <div key={i} className="rounded-xl p-3 border text-center" style={{ background: `${item.color}08`, borderColor: `${item.color}25` }}>
              ...
              <div className="text-xl font-bold nf-count" style={{ color: item.color }}>{item.val}</div>
      ...
      <RevealBlock delay={200}>
        <div className="mb-12">
          ...
          <h2 className="text-2xl font-bold text-text mb-6">Chuỗi Công Thức 10 Bước</h2>
      ...
      <RevealBlock delay={250}>
        <div className="mb-12">
          ...
          <h2 className="text-2xl font-bold text-text mb-6">Bộ Tính Từng Bước</h2>
          <div className="grid md:grid-cols-[220px_1fr] gap-4">
            {/* Step nav */}
            <div className="rounded-2xl border p-3 h-fit" style={{ background: '#0d0d0d', borderColor: '#1e1e1e' }}>
              {FORMULA_STEPS.map((step, i) => (
                <button key={i} onClick={() => setActiveStep(i)}
                  className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left transition-all mb-0.5 text-xs"
            ...
            <div className="rounded-2xl border p-5" style={{ background: '#0a0a0a', borderColor: '#1e1e1e' }}>
              <div className="flex items-center gap-3 mb-5">
                <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: `${GREEN}20`, color: GREEN }}>
                  {FORMULA_STEPS[activeStep].n}
                </div>
                <div>
                  <div className="font-bold text-text text-sm">{FORMULA_STEPS[activeStep].emoji} {FORMULA_STEPS[activeStep].label}</div>
      ...
      <RevealBlock delay={100}>
        <div className="mb-12">
          ...
          <h2 className="text-2xl font-bold text-text mb-6">Nước & Chất Xơ</h2>
      ...
      <RevealBlock delay={100}>
        <div className="mb-12">
          ...
          <h2 className="text-2xl font-bold text-text mb-2">Case Study — {CASE_STUDY.name}</h2>
          <p className="text-muted text-sm mb-6">{CASE_STUDY.activityLabel} · {CASE_STUDY.note}</p>
          ...
          <div className="grid md:grid-cols-[1fr_2fr] gap-4 mb-6">
            <div className="rounded-2xl p-4 border space-y-2" style={{ background: '#0d0d0d', borderColor: '#1e1e1e' }}>
              ...
              {[
                ...
                <div key={i} className="flex justify-between text-xs py-1 border-b" style={{ borderColor: '#1a1a1a' }}>
            ...
            <div className="rounded-2xl border overflow-hidden" style={{ borderColor: '#1e1e1e' }}>
              ...
              {[
                ...
                <div key={i} className="grid grid-cols-4 px-3 py-2 text-xs border-t" style={{ borderColor: '#1a1a1a', background: i % 2 === 0 ? 'transparent' : '#0d0d0d' }}>
          ...
          <div className="rounded-2xl border overflow-hidden" style={{ borderColor: '#1e1e1e' }}>
            ...
            {[
              ...
              <div key={i} className="grid grid-cols-5 px-3 py-2.5 text-xs border-t" style={{ borderColor: '#1a1a1a', background: i % 2 === 0 ? 'transparent' : '#0d0d0d' }}>
      ...
      <RevealBlock delay={100}>
        <div className="mb-12">
          ...
          <h2 className="text-2xl font-bold text-text mb-6">Quy Tắc Điều Chỉnh</h2>
            ...
            {[
              ...
              <div key={i} className="rounded-2xl p-4 border" style={{ background: `${card.color}08`, borderColor: `${card.color}25` }}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="font-bold text-sm mb-1" style={{ color: card.color }}>{card.title}</div>
                    <div className="text-xs text-muted leading-relaxed">{card.action}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <RevealBlock delay={100}>
        <div className="rounded-2xl p-5 border mb-8" style={{ background: 'rgba(132,204,22,0.05)', borderColor: 'rgba(132,204,22,0.25)' }}>
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-xl shrink-0" style={{ background: 'rgba(132,204,22,0.15)' }}>
              🛡️
            </div>
            <div>
