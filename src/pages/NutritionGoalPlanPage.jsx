    ...
    <div className="min-h-screen" style={{ background: '#0a0a0a', color: '#f5f5f5' }}>
      <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
        ...
        <RevealBlock className="mb-14">
          <div className="mb-2">
            <span
              className="text-base font-bold uppercase tracking-widest px-3 py-1 rounded-full border inline-block mb-4"
            ...
            {FIVE_QUESTIONS.map((q, i) => (
              <div key={q.n} className="flex-1 relative">
                ...
                <div className="flex flex-col items-center px-2 relative z-20">
                  ...
                  <div
                    className="w-6 h-6 rounded-full flex items-center justify-center text-base font-bold mb-3"
                    style={{ background: '#84cc16', color: '#000' }}
                  >
                    {q.n}
                  </div>
                  <p className="text-base font-bold text-center mb-1" style={{ color: '#f5f5f5' }}>{q.q}</p>
                  <p className="text-base text-center leading-relaxed" style={{ color: '#6b7280' }}>{q.detail}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-lg font-bold" style={{ color: '#f5f5f5' }}>{q.q}</span>
                  </div>
                  <p className="text-base leading-relaxed" style={{ color: '#6b7280' }}>{q.detail}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <RevealBlock className="mb-14">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {GOALS.map(g => (
              <button
                ...
                className="px-3 py-2 rounded-xl border text-base font-bold transition-all duration-200"
                style={{
          ...
          >
            {/* Formula note */}
            <div
              className="rounded-xl px-4 py-3 mb-5 text-base leading-relaxed border-l-4"
            ...
            >
              ...
              <br />
              <span className="font-mono text-base mt-1 block" style={{ color: '#6b7280' }}>
                {b0.sx === 'female'
                  ? `BMR = 10×${b0.w} + 6.25×${b0.h} − 5×${b0.age} − 161 = ${calcStats.bmr} kcal`
                  : `BMR = 10×${b0.w} + 6.25×${b0.h} − 5×${b0.age} + 5 = ${calcStats.bmr} kcal`}
              </span>
              <span className="font-mono text-base block" style={{ color: '#6b7280' }}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6" key={calcGoal}>
              {[
                ...
                >
                  ...
                  <span className="text-base font-semibold mb-2" style={{ color: m.color }}>{m.unit}</span>
                  <span className="text-base text-center leading-tight" style={{ color: '#6b7280' }}>{m.label}</span>
                </div>
              ))}
            </div>
            ...
            {GOALS.map(g => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-base font-bold leading-tight" style={{ color: active ? g.color : '#f5f5f5' }}>{g.label}</span>
                  <span
          ...
          {goalData && (
            ...
            >
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold mb-2" style={{ color: '#f5f5f5' }}>{goalData.tagline}</p>
                    <p className="text-lg leading-relaxed" style={{ color: '#9ca3af' }}>{goalData.who}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="relative h-44 md:h-56 overflow-hidden">
                ...
                <div
                  className="absolute bottom-4 left-5 text-base font-bold uppercase tracking-widest px-3 py-1 rounded-full border"
              ...
              <div className="p-6">
                ...
                >
                  <h4 className="text-lg font-bold mb-3" style={{ color: goalData.color }}>Macro cá nhân hóa cho bạn ({b0.w}kg)</h4>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4" key={`detail-${selectedGoal}`}>
                    {[
                      ...
                      <div key={m.label} className="flex flex-col items-center">
                        <span className="text-xl font-bold" style={{ color: m.color }}>{m.value}<span className="text-base ml-0.5">{m.unit}</span></span>
                        <span className="text-base mt-0.5" style={{ color: '#6b7280' }}>{m.label}</span>
                      </div>
                    ))}
                  </div>
                  <div className="grid grid-cols-2 gap-x-6" key={`pbars-${selectedGoal}`}>
                    <div>
                      <div className="text-base mb-1" style={{ color: '#6b7280' }}>Protein/ngày: {goalData.proteinRule}</div>
                      <div className="text-base" style={{ color: '#6b7280' }}>Fat/ngày: {goalData.fatRule}</div>
                    </div>
                    <div>
                      <div className="text-base mb-1" style={{ color: '#6b7280' }}>Carb/ngày: {goalData.carbRule}</div>
                      <div className="text-base" style={{ color: '#6b7280' }}>Nước: {detailStats.waterMl}ml/ngày</div>
                    </div>
                  </div>
                </div>

                {/* Meal structure */}
                <h4 className="text-lg font-bold uppercase tracking-wider mb-3" style={{ color: '#6b7280' }}>Thực đơn mẫu</h4>
                <div className="space-y-2 mb-6">
                  {goalData.meals.map((meal, mi) => (
                    ...
                    >
                      <span
                        className="shrink-0 text-base font-bold px-2 py-1 rounded-lg mt-0.5"
                        style={{ background: `rgba(${goalData.rgb},0.12)`, color: goalData.color, minWidth: 52, textAlign: 'center' }}
                      >
                        {meal.t}
                      </span>
                      <div className="flex-1">
                        <p className="text-lg leading-snug mb-1" style={{ color: '#d1d5db' }}>{meal.food}</p>
                        <span className="text-base italic" style={{ color: '#6b7280' }}>{meal.note}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {/* Progress signs */}
                <h4 className="text-lg font-bold uppercase tracking-wider mb-3" style={{ color: '#6b7280' }}>Dấu hiệu đang đi đúng hướng</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-6">
                  {goalData.signs.map((sign, si) => (
                    <div key={si} className="flex items-start gap-2">
                      <span className="shrink-0 text-lg mt-0.5" style={{ color: '#22c55e' }}>✓</span>
                      <span className="text-lg" style={{ color: '#d1d5db' }}>{sign}</span>
                    </div>
                  ))}
                </div>

                {/* Tips */}
                <h4 className="text-lg font-bold uppercase tracking-wider mb-3" style={{ color: '#6b7280' }}>Tips quan trọng</h4>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
                  {goalData.tips.map((tip, ti) => (
                    <div
                      ...
                      style={{ background: `rgba(${goalData.rgb},0.06)`, borderColor: `rgba(${goalData.rgb},0.18)` }}
                    >
                      <span className="text-base leading-relaxed" style={{ color: '#d1d5db' }}>{tip}</span>
                    </div>
                  ))}
                </div>

                {/* Weekly schedule */}
                <h4 className="text-lg font-bold uppercase tracking-wider mb-3" style={{ color: '#6b7280' }}>Khung tuần</h4>
                <div className="rounded-xl border overflow-hidden mb-6" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
                  {goalData.weekPlan.map((row, ri) => (
                    ...
                    >
                      <span
                        className="shrink-0 text-base font-bold px-2 py-1 rounded-lg"
                        style={{ background: `rgba(${goalData.rgb},0.12)`, color: goalData.color, minWidth: 60, textAlign: 'center' }}
                      >
                        {row.day}
                      </span>
                      <span className="text-lg" style={{ color: '#d1d5db' }}>{row.guide}</span>
                    </div>
                  ))}
                </div>

                {/* Adjustment rules */}
                <h4 className="text-lg font-bold uppercase tracking-wider mb-3" style={{ color: '#6b7280' }}>Quy tắc điều chỉnh</h4>
                <div className="rounded-xl border overflow-hidden" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
                  <div className="grid grid-cols-2 text-base font-bold px-3 py-2 border-b" style={{ borderColor: 'rgba(255,255,255,0.08)', color: '#6b7280', background: 'rgba(255,255,255,0.04)' }}>
                  ...
                  {goalData.adjustments.map((adj, ai) => (
                    <div
                      key={ai}
                      className="grid grid-cols-2 gap-4 px-3 py-3 border-b last:border-b-0 text-lg"
          ...
          >
            <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
              ...
              <div className="shrink-0 flex flex-col items-center gap-3">
                ...
                <div
                  className="text-base text-center px-3 py-2 rounded-xl border"
              ...
              <div className="flex-1">
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {[
                    ...
                    >
                      ...
                      <div>
                        <div className="text-lg font-bold" style={{ color: seg.color }}>{seg.label}</div>
                        <div className="text-base" style={{ color: '#6b7280' }}>{seg.sub}</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div
                  className="rounded-xl border-l-4 px-4 py-3 text-lg leading-relaxed"
        ...
        <RevealBlock className="mb-14">
          ...
          <p className="text-lg mb-6" style={{ color: '#9ca3af' }}>Không có thời gian lên kế hoạch? Dùng combo gợi ý này ngay.</p>
          ...
          >
            {/* Table header */}
            <div
              className="grid grid-cols-12 px-4 py-3 border-b text-base font-bold uppercase tracking-widest"
            ...
            {QUICK_COMBOS.map((row, ri) => (
              ...
              >
                <div className="col-span-3 flex items-center gap-2">
                  ...
                  <span className="text-lg font-semibold" style={{ color: '#f5f5f5' }}>{row.goal}</span>
                </div>
                <div className="col-span-9">
                  {row.combo.split('; ').map((c, ci) => (
                    <div key={ci} className="flex items-start gap-2 mb-1 last:mb-0">
                      ...
                      <span className="text-lg" style={{ color: '#d1d5db' }}>{c}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-semibold text-lg" style={{ color: open ? '#84cc16' : '#f5f5f5' }}>
                        {sig.trigger}
                      </span>
                    </div>
                    <span
                      ...
                      <ul className="space-y-2 mt-4">
                        {sig.items.map((item, ii) => (
                          <li key={ii} className="flex items-start gap-3">
                            <span className="shrink-0 mt-0.5 text-lg" style={{ color: '#84cc16' }}>→</span>
