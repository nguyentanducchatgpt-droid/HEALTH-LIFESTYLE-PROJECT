  ...
  return (
    <div className="mb-8 rounded-2xl p-5 border" style={{ borderColor: `rgba(${HEX_RGB},0.25)`, background: `rgba(${HEX_RGB},0.05)` }}>
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-lg font-semibold mb-1" style={{ color: C }}>Hồ sơ cá nhân của bạn</p>
          ...
          <p className="text-base mt-1" style={{ color: tier.color }}>
...
function StorageTimer() {
  ...
  return (
    <div className="rounded-2xl p-6 border border-sky-500/20 bg-sky-500/5">
      ...
      <div className="flex flex-col gap-3">
        {zones.map((z, i) => (
          <div key={i} className="flex items-center gap-4">
            <div className="w-24 text-base font-mono font-bold shrink-0" style={{ color: z.color }}>{z.label}</div>
            ...
            <div className="w-28 text-base shrink-0">
      ...
      <p className="text-base text-gray-500 mt-4">Nguồn: FoodSafety.gov & USDA FSIS — quy tắc 2 giờ + 3–4 ngày</p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl p-6 border border-sky-500/20 bg-sky-500/5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-lg font-bold px-3 py-1 rounded-full" style={{ background: `rgba(${HEX_RGB},0.15)`, color: C }}>{count}/{DAILY_CHECKS.length} · {pct}%</span>
      </div>
      ...
      <div className="space-y-2">
        {DAILY_CHECKS.map((q, i) => (
          <label key={i} className="flex items-start gap-3 cursor-pointer group select-none">
            ...
            <span className={`text-lg leading-relaxed transition-colors ${checked[i] ? 'text-gray-400 line-through' : 'text-gray-300'}`}>{q}</span>
          </label>
        ))}
      </div>
      {pct === 100 && (
        <div className="mt-4 p-3 rounded-xl text-center text-lg font-bold" style={{ background: `rgba(${HEX_RGB},0.15)`, color: C }}>
  ...
  return (
    <div className="rounded-2xl p-6 border border-sky-500/20 bg-sky-500/5">
      <div className="flex items-center justify-between mb-4">
        ...
        {bad >= 3 && (
          <span className="text-base font-bold px-2 py-1 rounded-full bg-red-500/20 text-red-400">⚠️ {bad} chỉ số xấu — giảm độ khó!</span>
        )}
      </div>
      <div className="space-y-3">
        {WEEKLY_CHECKS.map((q, i) => (
          <div key={i} className="flex items-center gap-3">
            <span className="text-lg text-gray-300 flex-1">{q}</span>
      ...
      {bad >= 3 && (
        <div className="mt-4 p-3 rounded-xl text-lg" style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)' }}>
    ...
    <main className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          ...
          <span className="inline-block text-base font-bold uppercase tracking-widest mt-3 mb-4 px-3 py-1 rounded-full border" style={{ color: C, background: `rgba(${HEX_RGB},0.1)`, borderColor: `rgba(${HEX_RGB},0.2)` }}>
      ...
      <div className="sf-orbit-ring rounded-3xl p-[1.5px] mb-12">
        <div className="relative rounded-3xl overflow-hidden h-52 md:h-72">
          ...
          <div className="absolute bottom-4 left-6">
            <span className="text-base font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: C, background: 'rgba(0,0,0,0.6)', borderColor: `rgba(${HEX_RGB},0.2)` }}>
      ...
      <RevealBlock className="mb-12">
        <div className="rounded-3xl p-8 text-center border animate-pulse-glow" style={{ background: `rgba(${HEX_RGB},0.06)`, borderColor: `rgba(${HEX_RGB},0.2)` }}>
          ...
          <p className="text-lg" style={{ color: C }}>— Kim chỉ nam của dự án Nutrition & Meal Plans</p>
        </div>
      </RevealBlock>
            ...
            <div key={i} className="flex gap-4 rounded-2xl p-5 border border-white/5 bg-white/[0.02] hover:border-sky-500/20 transition-all group">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-base font-bold rounded-full px-2 py-0.5" style={{ background: `rgba(${HEX_RGB},0.15)`, color: C }}>{i + 1}</span>
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-gray-400 text-lg mb-6">Trước khi áp dụng meal plan, xác định bạn thuộc nhóm nào để chọn mức độ phù hợp.</p>
          ...
          <div className={`rounded-2xl p-6 border ${activeTierData.bg} ${activeTierData.border} transition-all`}>
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <span className="text-base font-bold uppercase tracking-widest px-2 py-0.5 rounded-full"
                  style={{ background: `${activeTierData.color}20`, color: activeTierData.color }}>{activeTierData.badge}</span>
              </div>
            </div>
            <p className="text-gray-300 text-lg mb-4 leading-relaxed">{activeTierData.desc}</p>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          {[
            ...
            <div key={i} className="rounded-2xl p-5 border border-white/5 bg-white/[0.02]">
              ...
              <p className="text-lg text-gray-400 leading-relaxed">{item.rule}</p>
            </div>
          ))}
        </div>
        <div className="rounded-2xl p-5 border border-amber-500/20 bg-amber-500/5">
          <p className="text-lg text-amber-300"><strong>📌 CDC/NIH:</strong> Giảm cân từ từ ~0.5–1 kg/tuần bền vững hơn giảm nhanh. Thường đạt được với mức âm khoảng 500 kcal/ngày.</p>
        </div>
      </RevealBlock>
            ...
            <div key={i} className="rounded-2xl p-6 border border-white/5 bg-white/[0.02]">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="rounded-xl p-4 bg-green-500/5 border border-green-500/20">
                  <p className="text-base font-bold text-green-400 mb-1">✅ Nguyên tắc dự án</p>
                  <p className="text-lg text-gray-300">{m.safe}</p>
                </div>
                <div className="rounded-xl p-4 bg-red-500/5 border border-red-500/20">
                  <p className="text-base font-bold text-red-400 mb-1">⚠️ Thận trọng với</p>
                  <p className="text-lg text-gray-300">{m.caution}</p>
                </div>
              </div>
              <div className="mt-3 rounded-xl p-3 border" style={{ borderColor: `rgba(${HEX_RGB},0.2)`, background: `rgba(${HEX_RGB},0.05)` }}>
                <p className="text-base" style={{ color: C }}>💡 {m.tip}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <RevealBlock className="mb-12">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div>
            <p className="text-gray-400 text-lg mb-4 leading-relaxed">Đĩa ăn dự án an toàn hơn việc bắt người mới cân đo mọi thứ. Đây là công cụ mặc định.</p>
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center gap-3">
                  <span className="w-10 text-center font-bold text-lg" style={{ color: item.color }}>{item.pct}</span>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="rounded-xl p-3 bg-white/[0.02]">
                  <p className="text-lg font-bold text-white mb-1">{item.goal}</p>
                  <p className="text-base text-gray-400">{item.rule}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-gray-400 text-lg mb-6">4 nguyên tắc nền từ FoodSafety.gov: <strong className="text-white">Clean → Separate → Cook → Chill</strong></p>
          ...
          {STORAGE_RULES.map((r, i) => (
            <div key={i} className="rounded-2xl p-5 border border-white/5 bg-white/[0.02]">
              <div className="flex items-center gap-3 mb-3">
                ...
                <div>
                  <span className="text-base font-bold font-mono" style={{ color: r.color }}>{r.step}</span>
                  <h3 className="font-bold text-white">{r.title}</h3>
                </div>
              </div>
              <ul className="space-y-1">
                {r.items.map((item, j) => (
                  <li key={j} className="text-lg text-gray-400 flex items-start gap-2">
          ...
          <div className="rounded-2xl p-4 border border-red-500/20 bg-red-500/5">
            ...
            <div className="space-y-1">
              {['Mùi chua/lạ', 'Nhớt', 'Mốc', 'Hộp phồng', 'Cơm/sốt để ngoài lâu', 'Không nhớ nấu ngày nào'].map((s, i) => (
                <p key={i} className="text-base text-gray-400 flex items-center gap-2"><span className="text-red-400">✕</span> {s}</p>
              ))}
            </div>
          </div>
          ...
          {[
            ...
            <div key={i} className="rounded-2xl p-5 border border-white/5 bg-white/[0.02]">
              ...
              <ul className="space-y-2">
                {p.rules.map((r, j) => (
                  <li key={j} className="text-base text-gray-400 flex items-start gap-2"><span style={{ color: p.color }}>•</span>{r}</li>
                ))}
              </ul>
            </div>
          ))}
        ...
        <div className="rounded-2xl p-5 border border-amber-500/20 bg-amber-500/5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {['Tập tụt sức liên tục', 'Đói cồn cào về đêm', 'Mất ngủ', 'Chuột rút nhiều', 'Cáu gắt dễ', 'Nhịp tim nghỉ tăng', 'Đau cơ kéo dài', 'Sụt cân quá nhanh', 'Thèm ngọt mất kiểm soát'].map((s, i) => (
              <p key={i} className="text-base text-gray-400 flex items-center gap-1"><span className="text-amber-400">▸</span>{s}</p>
            ))}
          </div>
        </div>
      </RevealBlock>
      ...
      <RevealBlock className="mb-12">
        ...
        <div className="rounded-3xl p-6 border border-red-500/30 bg-red-500/5">
          <p className="text-lg text-red-300 mb-4 font-semibold">Dừng meal plan/tập luyện và đến cơ sở y tế ngay nếu xuất hiện bất kỳ dấu hiệu nào dưới đây:</p>
          ...
          <div className="mt-4 rounded-xl p-4 bg-white/5 border border-white/10 text-center">
            <p className="text-lg text-gray-300">Nguồn: <span className="text-white font-semibold">American Heart Association · ADA · WHO</span></p>
          </div>
        </div>
      </RevealBlock>
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-gray-400 text-lg mb-6">Dự án sống khỏe không được biến thành dự án ám ảnh hình thể. Nhấn vào thẻ để xem thông điệp tích cực thay thế.</p>
        <div className="space-y-3">
            ...
            <div key={i}
              ...
              <div className={`p-4 transition-all ${flipMsg === i ? 'bg-green-500/5' : 'bg-red-500/5'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-lg font-medium" style={{ color: flipMsg === i ? '#22c55e' : '#ef4444' }}>
                    {flipMsg === i ? m.good : m.bad}
                  </p>
                  <span className="ml-auto text-base text-gray-500">{flipMsg === i ? 'ĐÚNG ↑' : 'SAI ↓'}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {DONT_DO.map((d, i) => (
            <div key={i} className="rounded-2xl p-4 border border-red-500/20 bg-red-500/5 flex items-start gap-3 hover:border-red-500/40 transition-all">
              ...
              <p className="text-base text-gray-300 leading-relaxed">{d.text}</p>
            </div>
          ))}
        </div>
      </RevealBlock>
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-gray-400 text-lg mb-4">Cần hỏi tối thiểu 13 thông tin trước khi đưa ra kế hoạch ăn uống cá nhân hóa:</p>
        <div className="grid md:grid-cols-2 gap-3">
          {[
        ...
        <div className="mt-4 rounded-2xl p-4 border border-amber-500/20 bg-amber-500/5">
          <p className="text-lg text-amber-300"><strong>⚠️ Quan trọng:</strong> Nếu thiếu thông tin bệnh nền, thuốc, dị ứng hoặc triệu chứng nguy hiểm — không nên đưa meal plan cá nhân hóa sâu.</p>
        </div>
      </RevealBlock>
      ...
      <RevealBlock className="mb-12">
        ...
        <p className="text-gray-400 text-lg mb-4">Mỗi video hoặc bài viết Nutrition nên có 3 lớp an toàn:</p>
        <div className="space-y-4">
          {[
            {
            ...
            <div key={i} className="rounded-2xl p-5 border border-white/5 bg-white/[0.02]">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-base font-bold uppercase tracking-widest" style={{ color: l.color }}>{l.layer}</span>
                <h3 className="font-bold text-white">{l.title}</h3>
              </div>
              <p className="text-lg text-gray-400 italic leading-relaxed">{l.content}</p>
            </div>
          ))}
        </div>
      </RevealBlock>
      ...
      <RevealBlock>
        <div className="rounded-3xl p-8 border text-center" style={{ borderColor: `rgba(${HEX_RGB},0.25)`, background: `rgba(${HEX_RGB},0.05)` }}>
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <Link to="/pillar/b/checklist" className="px-5 py-2.5 rounded-full text-lg font-bold transition-all hover:scale-105"
              style={{ background: `rgba(${HEX_RGB},0.15)`, color: C, border: `1px solid rgba(${HEX_RGB},0.3)` }}>
              ✅ Checklist Dinh Dưỡng
            </Link>
            <Link to="/pillar/b/template" className="px-5 py-2.5 rounded-full text-lg font-bold transition-all hover:scale-105"
