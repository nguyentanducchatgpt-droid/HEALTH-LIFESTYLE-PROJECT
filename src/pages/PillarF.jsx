        ...
        <div className="flex-1 p-5 md:p-6 flex flex-col justify-between min-w-0">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: COLOR }}>{category}</span>
            </div>
            <h3 className="text-lg font-bold text-text mb-1 leading-tight">{title}</h3>
            <p className="text-xs text-muted mb-3">{accent}</p>
            <p className="text-sm text-muted leading-relaxed mb-4">{desc}</p>
            <ul className="space-y-1 mb-5">
              {features.map((f, i) => (
                <li key={i} className="flex gap-2 text-xs text-muted"><span style={{ color: COLOR }}>✦</span>{f}</li>
              ))}
            </ul>
          </div>
            ...
            <div className="flex gap-4">
              {stats.map((s, i) => (
                <div key={i} className="text-center">
                  <div className="text-xl font-black" style={{ color: COLOR }}>{s.v}</div>
                  <div className="text-xs text-muted">{s.l}</div>
                </div>
              ))}
            </div>
            <span className="ml-auto text-sm font-bold group-hover:translate-x-1 transition-transform" style={{ color: COLOR }}>{cta}</span>
          </div>
        </div>
  ...
  return (
    <div className="mb-12">
      <div className="flex items-center gap-3 mb-5">
        ...
        <span className="text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border" style={{ color: COLOR, borderColor: `rgba(${RGB},0.3)`, background: `rgba(${RGB},0.07)` }}>{title}</span>
        <div className="h-px flex-1 bg-border" />
      </div>
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-border bg-surface p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-right">
            <div className="text-3xl font-black" style={{ color: level.color }}>{total}</div>
        ...
        <div className="space-y-3">
          {SCORE_ITEMS.map((item, i) => (
            <div key={i} className="flex items-center gap-3">
              ...
              <span className="text-xs text-muted w-8 text-right">{item.pts}đ</span>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link to="/pillar/f/checklist" className="rounded-2xl border border-border bg-surface p-4 hover:border-orange-500/30 transition-colors text-center">
          ...
          <div className="text-xs font-bold text-text">Checklist Ngày</div>
        </Link>
        <Link to="/pillar/f/workout-log" className="rounded-2xl border border-border bg-surface p-4 hover:border-orange-500/30 transition-colors text-center">
          ...
          <div className="text-xs font-bold text-text">Ghi Nhật Ký</div>
        </Link>
        <Link to="/pillar/f/health-score" className="rounded-2xl border border-border bg-surface p-4 hover:border-orange-500/30 transition-colors text-center">
          <div className="text-2xl mb-1">💯</div>
          <div className="text-xs font-bold text-text">Lịch Sử Điểm</div>
        </Link>
        <Link to="/pillar/f/progress-test" className="rounded-2xl border border-border bg-surface p-4 hover:border-orange-500/30 transition-colors text-center">
          <div className="text-2xl mb-1">📈</div>
          <div className="text-xs font-bold text-text">Test Tiến Bộ</div>
        </Link>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      <div className="flex gap-2 mb-4">
        <button onClick={() => setWeekView(false)} className="flex-1 py-2 rounded-xl text-sm font-bold transition-colors" style={!weekView ? { background: COLOR, color: 'white' } : { background: 'var(--surface)', color: 'var(--muted)', border: '1px solid var(--border)' }}>Hôm Nay</button>
        <button onClick={() => setWeekView(true)} className="flex-1 py-2 rounded-xl text-sm font-bold transition-colors" style={weekView ? { background: COLOR, color: 'white' } : { background: 'var(--surface)', color: 'var(--muted)', border: '1px solid var(--border)' }}>Tuần Này</button>
      </div>
      {!weekView ? (
        <>
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm font-bold" style={{ color: COLOR }}>{doneCount}/{DAILY_MIN.length}</span>
          </div>
          <p className="text-xs text-muted mb-3">{msg} — Làm được 70% là thắng.</p>
          <div className="space-y-2">
            {DAILY_MIN.map((item, i) => (
              <button key={i} onClick={() => toggle(i)} className="w-full flex items-center gap-3 rounded-2xl border border-border bg-surface p-4 text-left hover:border-orange-500/30 transition-colors">
                ...
                <span className={`text-sm flex-1 ${checked[i] ? 'line-through text-muted' : 'text-text'}`}>{item.label}</span>
                <span className="text-xs px-1.5 py-0.5 rounded font-bold" style={{ background: `${CAT_COLORS[item.cat]}20`, color: CAT_COLORS[item.cat] }}>{item.cat}</span>
              </button>
            ))}
          </div>
        </>
        ...
        <div className="space-y-3">
          {WEEKLY_ITEMS.map((item, i) => (
            <div key={i} className="rounded-2xl border border-border bg-surface p-4 flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                <div className="text-xs text-muted">{item.target}</div>
              </div>
              <div className="flex gap-1">
          ...
          <p className="text-xs text-muted">Mỗi ô tương ứng 1 ngày trong tuần. Nhấn để tick.</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-border bg-surface p-4 space-y-3">
        <h3 className="font-bold text-text text-sm">Ghi Buổi Tập Mới</h3>
        <input value={form.exercise} onChange={e => setForm(p => ({ ...p, exercise: e.target.value }))} placeholder="Bài tập (vd: Full body 20 phút)" className="w-full bg-bg border border-border rounded-xl px-3 py-2 text-sm text-text placeholder-muted" />
        <div className="grid grid-cols-2 gap-2">
          <input value={form.duration} onChange={e => setForm(p => ({ ...p, duration: e.target.value }))} placeholder="Thời lượng (vd: 25 phút)" className="bg-bg border border-border rounded-xl px-3 py-2 text-sm text-text placeholder-muted" />
          <input value={form.feeling} onChange={e => setForm(p => ({ ...p, feeling: e.target.value }))} placeholder="Cảm giác (vd: Hơi mệt)" className="bg-bg border border-border rounded-xl px-3 py-2 text-sm text-text placeholder-muted" />
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-muted shrink-0">RPE: <strong style={{ color: RPE_COLORS[form.rpe] }}>{form.rpe}/10</strong></span>
          <input type="range" min={1} max={10} value={form.rpe} onChange={e => setForm(p => ({ ...p, rpe: +e.target.value }))} className="flex-1" />
        </div>
        <input value={form.next} onChange={e => setForm(p => ({ ...p, next: e.target.value }))} placeholder="Lần sau (vd: Tăng 2 lần squat)" className="w-full bg-bg border border-border rounded-xl px-3 py-2 text-sm text-text placeholder-muted" />
        <button onClick={add} className="w-full py-2 rounded-xl text-sm font-bold text-white" style={{ background: COLOR }}>+ Lưu Buổi Tập</button>
      </div>
      {entries.length === 0 && <p className="text-center text-muted text-sm py-6">Chưa có nhật ký. Ghi buổi tập đầu tiên.</p>}
      <div className="space-y-2 max-h-72 overflow-y-auto">
        {entries.map((e, i) => (
          <div key={i} className="rounded-xl border border-border bg-surface p-3">
            <div className="flex items-center gap-2 mb-1">
              <div className="w-2 h-2 rounded-full" style={{ background: RPE_COLORS[e.rpe] }} />
              <span className="text-sm font-bold text-text">{e.exercise}</span>
            ...
            {e.next && <p className="text-xs text-muted mt-1">Lần sau: {e.next}</p>}
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="space-y-5">
      <div className="rounded-2xl border border-border bg-surface p-4">
        ...
        <p className="text-xs text-muted">+ Một lượng nhỏ chất béo tốt: dầu olive, bơ, các loại hạt.</p>
      </div>
      <div className="rounded-2xl border border-border bg-surface overflow-hidden">
        <div className="grid grid-cols-3 text-xs font-bold uppercase tracking-widest p-3 border-b border-border" style={{ color: COLOR }}>
          <span>Bữa</span><span>Công thức</span><span>Ví dụ</span>
        </div>
        {MEALS.map((m, i) => (
          <div key={i} className="grid grid-cols-3 p-3 border-b border-border/50 text-xs">
      ...
      <div className="rounded-2xl border border-border bg-surface p-4">
        <h3 className="font-bold text-text mb-3 text-sm">Nhật Ký Dinh Dưỡng Hôm Nay</h3>
        <div className="space-y-2">
          {[['protein', 'Protein đủ?', '🥩'], ['veg', 'Rau đủ?', '🥦'], ['water', 'Nước đủ?', '💧'], ['sugar', 'Ít đồ ngọt?', '🍬']].map(([k, label, ic]) => (
            <div key={k} className="flex items-center gap-3">
              ...
              <div className="flex gap-2">
                {['Có', 'Chưa'].map(opt => (
                  <button key={opt} onClick={() => setVal(k, opt)} className="px-3 py-1 rounded-lg text-xs font-bold border transition-colors"
    ...
    <div className="space-y-4">
      {[
        ...
        <div key={item.key} className="rounded-2xl border border-border bg-surface p-4">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xl">{item.icon}</span>
            <span className="text-sm text-text font-medium flex-1">{item.label}</span>
            <span className="text-xl font-black" style={{ color: item.good(data[item.key]) }}>{data[item.key]}<span className="text-xs font-normal ml-0.5">{item.unit}</span></span>
          </div>
      ...
      <div className="rounded-2xl border p-3 text-xs text-muted" style={{ borderColor: `rgba(${RGB},0.2)`, background: `rgba(${RGB},0.04)` }}>
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-border bg-surface p-4">
        <div className="flex items-center gap-2 mb-2">
          <span>😓</span>
          <span className="text-sm text-text flex-1">Mức stress hôm nay</span>
      ...
      <div className="rounded-2xl border border-border bg-surface p-4">
        <span className="text-sm text-text block mb-3">Tâm trạng hôm nay</span>
      ...
      <div className="rounded-2xl border border-border bg-surface p-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm text-text">Đã có calm practice hôm nay?</span>
          <button onClick={() => set('calm', !data.calm)} className="px-4 py-1.5 rounded-xl text-sm font-bold transition-colors"
        ...
        <p className="text-xs text-muted">Thở chậm, thiền, đi bộ không điện thoại, journaling...</p>
      </div>
      <div className="rounded-2xl border border-border bg-surface p-4">
        <span className="text-sm text-text block mb-2">Ghi nhanh 1 dòng</span>
        <textarea value={data.journal} onChange={e => set('journal', e.target.value)} rows={2} placeholder="Hôm nay tôi cảm thấy... / Điều làm tôi khó tập trung là..." className="w-full bg-bg border border-border rounded-xl px-3 py-2 text-sm text-text placeholder-muted resize-none" />
      </div>
    </div>
  );
}
...
function F6Test() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted">Thực hiện test đầu vào và mỗi 4 tuần một lần để đo tiến bộ toàn diện.</p>
      <div className="space-y-2">
        {TEST_ITEMS.map((item, i) => (
          <div key={i} className="rounded-2xl border border-border bg-surface p-3 flex items-center gap-3">
            <div className="flex-1">
              ...
              <div className="text-xs text-muted">{item.how}</div>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <input value={vals[item.label] || ''} onChange={e => set(item.label, e.target.value)} placeholder="0" className="w-16 bg-bg border border-border rounded-lg px-2 py-1 text-sm text-text text-center" />
              <span className="text-xs text-muted w-16">{item.unit}</span>
            </div>
          </div>
        ))}
      </div>
      <button onClick={save} className="w-full py-2 rounded-xl text-sm font-bold text-white transition-all" style={{ background: saved ? '#22c55e' : COLOR }}>
        {saved ? '✓ Đã lưu!' : 'Lưu Kết Quả Test'}
      </button>
      <Link to="/pillar/f/progress-test" className="block text-center text-xs" style={{ color: COLOR }}>Xem bảng so sánh 4 tuần →</Link>
    </div>
  );
}
...
function F7QuickWorkouts() {
  ...
  return (
    <div className="space-y-3">
      <p className="text-sm text-muted">Nguyên tắc: Có 5 phút vẫn làm được. Không bỏ hẳn — chỉ cần chọn bản ngắn hơn.</p>
      {QUICK_WO.map((wo, i) => (
        <div key={i} className="rounded-2xl border border-border bg-surface overflow-hidden">
          <button onClick={() => setOpen(open === i ? -1 : i)} className="w-full flex items-center gap-3 p-4 text-left hover:bg-white/5 transition-colors">
            <span className="font-black text-sm w-16" style={{ color: wo.color }}>{wo.dur}</span>
            <span className="font-bold text-sm text-text flex-1">{wo.label}</span>
          ...
          {open === i && (
            <div className="px-4 pb-4 border-t border-border pt-3 space-y-2">
              ...
              <p className="text-xs text-muted border-l-2 pl-2 mt-2" style={{ borderColor: wo.color }}>{wo.note}</p>
            </div>
          )}
        </div>
      ))}
      <Link to="/pillar/f/quick-workouts" className="block text-center text-xs py-2" style={{ color: COLOR }}>Xem đầy đủ thư viện bài nhanh →</Link>
    </div>
  );
}
  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <RevealBlock key={tab} delay={0} className="mb-14">
        <div className="rounded-2xl border p-5 md:p-6" style={{ borderColor: `rgba(${RGB},0.15)`, animation: `pb-frame-f0 4s ease-in-out infinite` }}>
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="text-lg font-bold text-text">{TABS.find(t => t.id === tab)?.label}</h2>
          </div>
          {tabContent[tab]}
        </div>
      </RevealBlock>
      ...
      <RevealBlock delay={1} className="mb-10">
        <h2 className="text-2xl md:text-3xl font-bold text-text mb-2">Công Cụ & Tài Nguyên Chi Tiết</h2>
        <p className="text-muted text-sm mb-10">11 chuyên đề chuyên sâu để bạn xây hệ thống sống khỏe hoàn chỉnh.</p>
      ...
      <p className="text-xs text-muted text-center">Mọi dữ liệu được lưu cục bộ trong thiết bị của bạn, không gửi lên server. Công cụ không thay thế tư vấn y tế chuyên nghiệp.</p>
    </div>
  );
}
