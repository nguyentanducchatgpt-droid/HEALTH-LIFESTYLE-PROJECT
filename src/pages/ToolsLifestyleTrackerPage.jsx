        ...
        <RevealBlock delay={0} className="mb-8">
          <div className="grid grid-cols-2 gap-4 p-5 rounded-2xl border" style={{ borderColor: `rgba(${RGB},0.2)`, background: `rgba(${RGB},0.05)` }}>
            <div className="text-center">
              ...
              <div className="text-sm text-muted">giờ ngủ TB/ngày</div>
              <div className="text-sm mt-1" style={{ color: +avgSleep >= 7 ? '#22c55e' : '#f59e0b' }}>{+avgSleep >= 7 ? '✓ Đủ giấc' : '⚠ Cần ngủ thêm'}</div>
            </div>
            <div className="text-center">
              <div className="text-4xl font-black" style={{ color: COLOR }}>{avgEnergy}</div>
              <div className="text-sm text-muted">năng lượng TB/10</div>
              <div className="text-sm mt-1" style={{ color: +avgEnergy >= 6 ? '#22c55e' : '#f59e0b' }}>{+avgEnergy >= 6 ? '✓ Tốt' : '⚠ Cần cải thiện'}</div>
            </div>
          </div>
        </RevealBlock>
      )}
        ...
        <div className="rounded-2xl border border-border bg-surface p-5 space-y-6">
          ...
          <div>
            <div className="flex justify-between mb-2">
              <label className="text-base font-medium text-text">😴 Số giờ ngủ đêm qua</label>
              <span className="text-base font-bold" style={{ color: COLOR }}>{data.sleep} giờ</span>
            </div>
            <input type="range" min="3" max="12" step="0.5" value={data.sleep} onChange={e => update('sleep', +e.target.value)}
              className="w-full" style={{ accentColor: COLOR }} />
            <div className="flex justify-between text-sm text-muted mt-1"><span>3h</span><span>7h (mục tiêu)</span><span>12h</span></div>
          </div>

          {/* Sleep quality */}
          <div>
            <label className="text-base font-medium text-text block mb-3">⭐ Chất lượng giấc ngủ</label>
            <div className="flex gap-2 flex-wrap">
              {SLEEP_QUALITY.map(q => (
                <button key={q.val} onClick={() => update('sleepQ', q.val)}
                  className="flex-1 flex flex-col items-center gap-1 py-2 px-1 rounded-xl border transition-all text-sm"
          ...
          <div className="grid grid-cols-2 gap-3">
            {[['🌙 Giờ ngủ', 'bedtime'], ['☀️ Giờ dậy', 'waketime']].map(([lbl, key]) => (
              <div key={key}>
                <label className="text-base font-medium text-text block mb-2">{lbl}</label>
          ...
          {/* Steps */}
          <div>
            <label className="text-base font-medium text-text block mb-2">🚶 Bước chân hôm nay</label>
            <input type="number" min="0" value={data.steps ?? ''} onChange={e => update('steps', e.target.value)}
              placeholder="vd: 7500" className="w-full px-3 py-2 rounded-xl border bg-transparent text-base text-text placeholder-muted focus:outline-none"
              style={{ borderColor: `rgba(${RGB},0.3)` }} />
          </div>
          ...
          <div>
            <div className="flex justify-between mb-2">
              <label className="text-base font-medium text-text">⚡ Mức năng lượng hôm nay</label>
              <span className="text-base font-bold" style={{ color: COLOR }}>{data.energy}/10</span>
            </div>
          ...
          {/* Notes */}
          <div>
            <label className="text-base font-medium text-text block mb-2">📝 Ghi chú hôm nay</label>
            <textarea value={data.notes ?? ''} onChange={e => update('notes', e.target.value)}
              rows={2} placeholder="Điều gì ảnh hưởng đến giấc ngủ/năng lượng hôm nay?" className="w-full px-3 py-2 rounded-xl border bg-transparent text-base text-text placeholder-muted resize-none focus:outline-none"
              style={{ borderColor: `rgba(${RGB},0.3)` }} />
          </div>
        </div>
      </RevealBlock>
        ...
        <div className="space-y-2">
          {STEPS_RANGES.map((r, i) => (
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl border border-border">
              ...
              <span className="text-base font-medium text-text w-32">{r.label}</span>
