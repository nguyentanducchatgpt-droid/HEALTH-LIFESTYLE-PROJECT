  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <div className="flex items-start gap-6 mb-10 relative">
        ...
        <div>
          <h1 className="text-4xl md:text-5xl font-bold text-text leading-tight">Template Thực Đơn</h1>
      ...
      <RevealBlock delay={0} className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-2" style={{ color: COLOR }}>Mô Hình Đĩa Ăn Chuẩn</h2>
        <p className="text-muted text-sm mb-6">Áp dụng cho tất cả các bữa chính — không cần cân đo, chỉ cần nhìn đĩa.</p>
        <div className="grid sm:grid-cols-3 gap-4">
          {PLATE_SECTIONS.map((s, i) => (
            <div key={i} className="rounded-2xl border bg-surface p-5 text-center" style={{ borderColor: `${s.color}30` }}>
              <div className="text-4xl font-black mb-2" style={{ color: s.color }}>{s.pct}%</div>
              <div className="font-bold text-text mb-2 text-sm">{s.label}</div>
              <div className="text-xs text-muted leading-relaxed">{s.desc}</div>
            </div>
          ))}
        </div>
        <div className="mt-4 p-4 rounded-2xl text-sm text-muted" style={{ background: `rgba(${RGB},0.05)`, borderLeft: `3px solid ${COLOR}` }}>
      ...
      <RevealBlock delay={1} className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-2" style={{ color: COLOR }}>Gợi Ý Bữa Ăn Việt</h2>
        <p className="text-muted text-sm mb-6">Click để xem gợi ý cho từng bữa. Ăn đa dạng, không ăn đi ăn lại 1 món.</p>
            ...
            <div key={i} className="rounded-2xl border border-border bg-surface overflow-hidden">
              <button onClick={() => setOpenMeal(openMeal === i ? null : i)} className="w-full flex items-center gap-4 p-4 text-left hover:bg-white/5 transition-colors">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted">4 gợi ý · nhấn để xem</div>
                </div>
      ...
      <RevealBlock delay={2} className="mb-12">
        <h2 className="text-xl md:text-2xl font-bold mb-2" style={{ color: COLOR }}>Nhật Ký Hôm Nay</h2>
        <p className="text-muted text-sm mb-6">Ghi nhanh — không cần hoàn hảo, chỉ cần ý thức về những gì đã ăn.</p>
          ...
          {NUTRITION_FIELDS.map(f => (
            <div key={f.key}>
              ...
              <input type="text" value={log[f.key] ?? ''} onChange={e => updateLog(f.key, e.target.value)}
                placeholder={f.placeholder} className="w-full px-3 py-2 rounded-xl border bg-transparent text-sm text-text placeholder-muted focus:outline-none"
                style={{ borderColor: `rgba(${RGB},0.3)` }} />
            </div>
          ))}
          <div>
            <label className="text-sm font-medium text-text block mb-2">📝 Ghi chú bữa ăn ngày hôm nay</label>
            <textarea value={log.notes ?? ''} onChange={e => updateLog('notes', e.target.value)}
              rows={3} placeholder="Ăn gì, cảm giác thế nào, muốn điều chỉnh gì..." className="w-full px-3 py-2 rounded-xl border bg-transparent text-sm text-text placeholder-muted resize-none focus:outline-none"
              style={{ borderColor: `rgba(${RGB},0.3)` }} />
          </div>
        </div>
      </RevealBlock>
