  ...
  return (
    <div className="px-4 md:px-6 max-w-4xl mx-auto pt-28 md:pt-32 pb-24">
      ...
      <RevealBlock delay={0} className="mb-12">
        ...
        <div className="rounded-2xl border border-border bg-surface p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-muted block mb-1">Ngày</label>
            ...
            <div>
              <label className="text-sm text-muted block mb-1">Bài tập</label>
              <div className="flex gap-2">
                <input type="text" value={form.exercise} onChange={e => setForm(f => ({ ...f, exercise: e.target.value }))}
                  placeholder="Tên bài tập" className="flex-1 px-3 py-2 rounded-xl border bg-transparent text-base text-text placeholder-muted focus:outline-none"
                  style={{ borderColor: `rgba(${RGB},0.3)` }} />
                <button onClick={() => setShowTemplate(o => !o)} className="px-3 py-2 rounded-xl text-sm font-bold border" style={{ color: COLOR, borderColor: `rgba(${RGB},0.3)` }}>
                  Template
                </button>
              </div>
            </div>
            ...
            <div className="rounded-xl border p-3 grid grid-cols-2 gap-2" style={{ borderColor: `rgba(${RGB},0.2)`, background: `rgba(${RGB},0.05)` }}>
              {EXERCISE_TEMPLATES.map((t, i) => (
                <button key={i} onClick={() => applyTemplate(t)}
                  ...
                  <div className="text-base font-medium text-text">{t.name}</div>
                  <div className="text-sm text-muted">{t.muscles}</div>
                </button>
              ))}
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-3">
            {[['Sets', 'sets', 'số set'], ['Reps', 'reps', 'số lần'], ['Cân nặng (kg)', 'weight', '0 = bodyweight']].map(([lbl, key, ph]) => (
              <div key={key}>
                <label className="text-sm text-muted block mb-1">{lbl}</label>
                <input type="number" min="0" value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                  placeholder={ph} className="w-full px-3 py-2 rounded-xl border bg-transparent text-base text-text placeholder-muted focus:outline-none"
                  style={{ borderColor: `rgba(${RGB},0.3)` }} />
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-sm text-muted block mb-1">Thời gian (phút)</label>
              <input type="number" min="0" value={form.duration} onChange={e => setForm(f => ({ ...f, duration: e.target.value }))}
                placeholder="tổng thời gian buổi tập" className="w-full px-3 py-2 rounded-xl border bg-transparent text-base text-text placeholder-muted focus:outline-none"
                style={{ borderColor: `rgba(${RGB},0.3)` }} />
            </div>
            <div>
              <label className="text-sm text-muted block mb-1">RPE: {form.rpe}/10 — {RPE_LABELS[form.rpe]}</label>
          ...
          <div>
            <label className="text-sm text-muted block mb-1">Ghi chú (tuần sau tăng tiến thế nào?)</label>
            <textarea value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))}
              rows={2} placeholder="vd: tăng 2.5kg hoặc thêm 1 set" className="w-full px-3 py-2 rounded-xl border bg-transparent text-base text-text placeholder-muted resize-none focus:outline-none"
              style={{ borderColor: `rgba(${RGB},0.3)` }} />
          </div>

          <button onClick={saveLog} className="w-full py-3 rounded-xl font-bold text-base text-white" style={{ background: COLOR }}>
            💾 Lưu buổi tập
          </button>
        </div>
      </RevealBlock>
      ...
      <RevealBlock delay={1} className="mb-12">
        ...
        {logs.length === 0 ? (
          <div className="text-center text-muted text-base py-8">Chưa có buổi tập nào. Hãy ghi buổi đầu tiên!</div>
        ) : (
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-sm px-2 py-0.5 rounded-full font-bold" style={{ color: RPE_COLORS[log.rpe], background: `${RPE_COLORS[log.rpe]}15` }}>
                        RPE {log.rpe}
                      </span>
                    </div>
                    ...
                    {log.note && <div className="text-sm text-muted mt-1 italic">→ {log.note}</div>}
                  </div>
                  <button onClick={() => deleteLog(log.id)} className="text-sm text-muted hover:text-red-400 transition-colors">✕</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <RevealBlock delay={2} className="mb-10">
        <div className="rounded-2xl border p-5" style={{ borderColor: `rgba(${RGB},0.2)`, background: `rgba(${RGB},0.05)` }}>
          ...
          <p className="text-sm text-muted mt-3">Mục tiêu thông thường: RPE 6–8 cho tập sức mạnh, RPE 4–6 cho cardio.</p>
        </div>
      </RevealBlock>
